...
if (!class_exists('ControllerModuleBlog')) {
	class ControllerModuleBlog extends Controller
	{
		...
		public function index($arg = array()) {
			...
			$module_data             = array();
            /*
			if (SC_VERSION < 20 || SC_VERSION > 23) {
		...
		public function avatar() {
			...
			if (isset($this->request->post['layout_id'])) {
				...
				$this->request->get['route'] = $this->seocmslib->getRouteByLayoutId($this->data['layout_id']);
			} else {
				$this->data['layout_id'] = '';
			}
			...
			if (isset($this->data['settingswidget']['store']) && in_array($this->config->get('config_store_id'), $this->data['settingswidget']['store'])) {
				if ($validate_cmswidget && isset($this->data['settingswidget']['customer_groups_avatar']) && !empty($this->data['customer_intersect'])) {
                    ...
                    /*
					if (file_exists(DIR_TEMPLATE . $this->seocmslib->theme_folder . '/template/agootemplates/module/' . $template)) {
						$this->template = $this->seocmslib->theme_folder . '/template/agootemplates/module/' . $template;
					} else {
			...
			if ($this->data['action'] != 'delete') {
				...
				if (!$json) {
					if (is_uploaded_file($this->request->files['file']['tmp_name']) && file_exists($this->request->files['file']['tmp_name'])) {
						...
						$this->load->model('tool/image');
						$json['file'] = $avatar_thumb = $this->seocmslib->resizeavatar($file, $new_filename, $width, $height, true, false);
			...
			if (empty($row)) {
				...
				if (isset($comments) && count($comments) > 0) {
					foreach ($comments as $comment) {
						...
						$parser->addBBCode("b", '<b>{param}</b>', false, false);
						...
						$text = str_replace(':(', '<img src="catalog/view/javascript/wysibb/theme/default/img/smiles/sm2.png">', $text);
						$text = str_replace(':D', '<img src="catalog/view/javascript/wysibb/theme/default/img/smiles/sm3.png">', $text);
						$text = str_replace(';)', '<img src="catalog/view/javascript/wysibb/theme/default/img/smiles/sm4.png">', $text);
						$text = str_replace(':up:', '<img src="catalog/view/javascript/wysibb/theme/default/img/smiles/sm5.png">', $text);
						$text = str_replace(':down:', '<img src="catalog/view/javascript/wysibb/theme/default/img/smiles/sm6.png">', $text);
						$text = str_replace(':shock:', '<img src="catalog/view/javascript/wysibb/theme/default/img/smiles/sm7.png">', $text);
						$text = str_replace(':angry:', '<img src="catalog/view/javascript/wysibb/theme/default/img/smiles/sm8.png">', $text);
	...
	public function wysibbuploader() {
	    ...
	    if (isset($this->request->post["iframe"])) {
			$isIframe =($this->request->post["iframe"]) ? true:false;
		} else {
			$isIframe = false;
		}
