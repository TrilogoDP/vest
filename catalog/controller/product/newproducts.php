	...
	class ControllerProductNewProducts extends Controller {
        ...
        $data['button_popup_view'] = $this->language->get('button_popup_view');
			...
			if (isset($this->request->get['limit'])) {
				...
				$limit = 100;//$this->config->get($this->config->get('config_theme') . '_product_limit')
			}
			...
			$this->document->setCanonical($this->url->link('product/newproducts'));
        ...
        'oct_product_stickers' => $oct_product_stickers,
			...
			*/
			...
			'value' => 'p.date_added',
			'href'  => $this->url->link('product/special', 'sort=p.date_added' . $url)
			);
			
			$data['sorts'][] = array(
			'text'  => $this->language->get('text_price_asc'),
			'value' => 'ps.price-ASC',
			'href'  => $this->url->link('product/newproducts', 'sort=ps.price&order=ASC' . $url)
			);
			
			$data['sorts'][] = array(
			'text'  => $this->language->get('text_price_desc'),
			'value' => 'ps.price-DESC',
			'href'  => $this->url->link('product/newproducts', 'sort=ps.price&order=DESC' . $url)
			);
			...
			*/
			...
			$pagination->total = 300; //$product_total;
			...
			if ($page > 1){
					$this->document->setCanonical($this->url->link('product/newproducts', 'page=' . ($page)));
