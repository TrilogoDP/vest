<?php
	class ControllerProductAmpProduct extends Controller {
		...
		private function ampfy($html) {
			...
			preg_match_all('/<iframe[^>]+src="([^"]+)"/', $html, $match);
			if (isset($match[1]) && $match[1] && isset($match[1][0]) && $match[1][0] && stripos($match[1][0], 'youtube')){
				$youtube_id = str_replace('//www.youtube.com/embed/', '', $match[1][0]);
				
				if ($youtube_id){
					$html = $html . '<amp-youtube data-videoid="' . $youtube_id . '" layout="responsive"  width="480" height="270"></amp-youtube>';
