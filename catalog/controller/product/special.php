	...
	class ControllerProductSpecial extends Controller {
		...
		if (isset($oct_data['oct_lazyload']) && $oct_data['oct_lazyload'] == 1) {
			...
			if (isset($this->request->get['limit'])) {
				...
				$limit = 20;//$this->config->get($this->config->get('config_theme') . '_product_limit')
			}
			...
			//TODO Galaxy-IT JAKIM START after
			$this->document->setDescription(sprintf($this->language->get('meta_description'), $this->config->get('config_telephone')));
			$this->document->setCanonical($this->url->link('product/special'));
			//TODO Galaxy-IT JAKIM END after
			...
			if ($category_id) {
				...
				if ($category){
					$data['heading_title'] = str_replace('Товары', $category['name'], $data['heading_title']);
					$data['heading_title'] = str_replace('Товари', $category['name'], $data['heading_title']);
				}
			}
        ...
        // oct_advanced_attributes_settings end
			...
			*/
			...
			'value' => 'ps.price-ASC',
			'href'  => $this->url->link('product/special', $url . '&sort=ps.price&order=ASC' . $url)
			);
			
			$data['sorts'][] = array(
			'text'  => $this->language->get('text_price_desc'),
			'value' => 'ps.price-DESC',
			'href'  => $this->url->link('product/special', $url . '&sort=ps.price&order=DESC' . $url)
			);
			...
			if ($this->config->get('config_review_status')) {
				...
				'value' => 'rating-DESC',
				'href'  => $this->url->link('product/special', $url . '&sort=rating&order=DESC' . $url)
				);
			...
			*/
			...
			if ($page > 1){
				$this->document->setCanonical($this->url->link('product/special', 'page=' . ($page)));
