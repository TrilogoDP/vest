<?php
	class ControllerHobotixHoboFAQ2 extends Controller {
		...
		public function index(){
			$this->load->model('hobotix/hobofaq2');
			...
			if (isset($this->request->get['route'])){	
				...
				if ($this->request->get['route'] == 'product/category'){
					...
					if ($category_info = $this->model_catalog_category->getCategory($category_id)){											
						...
						//Самый дешевый
						if ($product = $this->model_hobotix_hobofaq2->getCheapestProductsForCategory($category_id)){
							$data['seo_table'][] = $this->makeProductTableLine($data['text_seo_price_table_cheapest'], $product);
						}
						
						//Самый дорогой
						if ($product = $this->model_hobotix_hobofaq2->getExpensiveProductsForCategory($category_id)){
							$data['seo_table'][] = $this->makeProductTableLine($data['text_seo_price_table_expensive'], $product);							
						}
						
						//Самый просматриваемый
						if ($product = $this->model_hobotix_hobofaq2->getMostPopularProductsForCategory($category_id)){
							$data['seo_table'][] = $this->makeProductTableLine($data['text_seo_price_table_popular'], $product);					
						}
						
						//Самый обсуждаемый
						if ($product = $this->model_hobotix_hobofaq2->getMostReviewsProductsForCategory($category_id)){
							$data['seo_table'][] = $this->makeProductTableLine($data['text_seo_price_table_reviewed'], $product);						
						}
						
						//Самый новый
						if ($product = $this->model_hobotix_hobofaq2->getNewestProductsForCategory($category_id)){
					...
					/*		
						...
						if ($products = $this->model_hobotix_hobofaq2->getMostPopularProductsForCategory($category_id, 3)){							
						...
						//Новинки
						if ($products = $this->model_hobotix_hobofaq2->getNewestProductsForCategory($category_id, 3)){							
						...
						//Дешевые
						if ($products = $this->model_hobotix_hobofaq2->getCheapestProductsForCategory($category_id, 3)){
						...
						//Бестселлеры
						if ($products = $this->model_hobotix_hobofaq2->getBestSellerProductsForCategory($category_id, 3)){
						...
						if (!$ocfilter){
							//Ручной FAQ
							if ($handmade_faq = $this->model_hobotix_hobofaq2->getCategoryFaq($category_id)){
					...
					if ($manufacturer_info = $this->model_catalog_manufacturer->getManufacturer($manufacturer_id)){
						...
						//Самый дешевый
						if ($product = $this->model_hobotix_hobofaq2->getCheapestProductsForManufacturer($manufacturer_id)){
						$data['seo_table'][] = $this->makeProductTableLine($data['text_seo_price_table_cheapest'], $product);
					}
					
					//Самый дорогой
					if ($product = $this->model_hobotix_hobofaq2->getExpensiveProductsForManufacturer($manufacturer_id)){
						$data['seo_table'][] = $this->makeProductTableLine($data['text_seo_price_table_expensive'], $product);							
					}
					
					//Самый просматриваемый
					if ($product = $this->model_hobotix_hobofaq2->getMostPopularProductsForManufacturer($manufacturer_id)){
						$data['seo_table'][] = $this->makeProductTableLine($data['text_seo_price_table_popular'], $product);					
					}
					
					//Самый обсуждаемый
					if ($product = $this->model_hobotix_hobofaq2->getMostReviewsProductsForManufacturer($manufacturer_id)){
						$data['seo_table'][] = $this->makeProductTableLine($data['text_seo_price_table_reviewed'], $product);						
					}
					
					//Самый новый
					if ($product = $this->model_hobotix_hobofaq2->getNewestProductsForManufacturer($manufacturer_id)){
						$data['seo_table'][] = $this->makeProductTableLine($data['text_seo_price_table_newest'], $product);						
					}
					...
					//Самые просматриваемые
					if ($products = $this->model_hobotix_hobofaq2->getMostPopularProductsForManufacturer($manufacturer_id, 3)){							
					...
					//Новинки
					if ($products = $this->model_hobotix_hobofaq2->getNewestProductsForManufacturer($manufacturer_id, 3)){							
					...
					//Бестселлеры
					if ($products = $this->model_hobotix_hobofaq2->getBestSellerProductsForManufacturer($manufacturer_id, 3)){
					...
					//Ручной FAQ
					if ($handmade_faq = $this->model_hobotix_hobofaq2->getManufacturerFaq($manufacturer_id)){
						foreach ($handmade_faq as $handmade_qa){								
							$data['faq'][] = array(
							'question' 	=> $handmade_qa['question'],
							'answer'	=> $handmade_qa['answer']
					...
					if ($product_info = $this->model_catalog_product->getProduct($product_id)){
						...
						//Ручной FAQ
						if ($handmade_faq = $this->model_hobotix_hobofaq2->getProductFaq($product_id)){
							foreach ($handmade_faq as $handmade_qa){								
								$data['faq'][] = array(
								'question' 	=> $handmade_qa['question'],
								'answer'	=> $handmade_qa['answer']
					...
					if ($information_info = $this->model_catalog_information->getInformation($information_id)){
						...
						//Ручной FAQ
						if ($handmade_faq = $this->model_hobotix_hobofaq2->getInformationFaq($information_id)){
							foreach ($handmade_faq as $handmade_qa){								
								$data['faq'][] = array(
								'question' 	=> $handmade_qa['question'],
								'answer'	=> $handmade_qa['answer']
