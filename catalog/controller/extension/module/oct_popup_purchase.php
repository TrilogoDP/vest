	...
	class ControllerExtensionModuleOctPopupPurchase extends Controller {
		...
		public function make_order() {
			...
			if (isset($this->request->post['telephone'])) {
				if ((isset($oct_popup_purchase_data['telephone']) && $oct_popup_purchase_data['telephone'] == 2)) {
			...
			if (!isset($json['error'])) {
				...
				foreach ($results as $result) {
					if ($this->config->get($result['code'] . '_status') && $result['code'] != 'payment_feediscount') {
				...
				$json['telephone'] 	= $this->request->request['telephone'];


				$this->load->model('customer/customer');
				$json['customerData'] = [
					'email' 	=> $this->customer->getEmail(),					
					'new'		=> ((int)$this->model_customer_customer->getCountOrdersByCustomer(['email' => $this->customer->getEmail(), 'telephone' => $this->customer->getTelephone()]) >= 2)?'false':'true',
					'name' 		=> $this->customer->getFirstName(),
					'country'	=> 'Україна'
				];

				$json['customerData'] = array_map('prepareEcommString', $json['customerData']);
			}
