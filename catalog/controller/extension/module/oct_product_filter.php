...
class ControllerExtensionModuleOctProductFilter extends Controller {
    ...
    public function index() {
        //require_once(DIR_SYSTEM.'../ChromePhp.php');//***
        $data = array();
        ...
        //***mf begin
        if(strpos($this->request->server['REQUEST_URI'], "/ua/" !== false) || (isset($post_data['lang_mf']) && $post_data['lang_mf'] == 'ua')){//18-12-2018
            $language_id = 3;
        }     
        //***mf end
        ...
        //if(strpos($_SERVER['QUERY_STRING'], "/filter/") !== false){ $data['filter_start'] = 0;}//***
        ...
        if ($oct_product_filter_data['standard_status']) {
            ...
            if(isset($post_data['cat_id']) && (int)$post_data['cat_id'] > 0){
                $cat_id = (int)$post_data['cat_id'];
            }
            $standard_data = $this->model_extension_module_oct_product_filter->getFilterStandards($this->global_id, $this->global_type, $store_id, $customer_group_id, $language_id, $lead_time_status, $cat_id);
            //***mf end
    ...
    public function filterProducts() {
        ...
        //***mf begin
        if(isset($post_data['lang_mf'])){
            $json['lang_mf'] = $post_data['lang_mf'];
            if(isset($post_data['lang_mf']) && $post_data['lang_mf'] == 'ua'){
                $json['lang_mf'] = $post_data['lang_mf'];
                if(strpos($json['url'], "/ua/") === false){
                     $json['url'] = str_replace("vest.in.ua/","vest.in.ua/ua/",$json['url']);
                }
            }
        }
        //***mf end
            ...
            if ($results) {
                foreach ($results as $result) {
                    if ($result) {
                        ...
                        $result['description'] = $this->model_tool_mikrof->formatStringdisplay($result['description']);//***mf
