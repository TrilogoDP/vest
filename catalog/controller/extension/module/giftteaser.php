<?php 
	class ControllerExtensionModuleGiftTeaser extends Controller  {
		...
		public function checkGiftsForCurrentProduct($input = array()){
			$this->load->model('catalog/product');
			$data['return_data'] = $input['return_data'];
			...
			if (empty($giftTeaser[$this->moduleName]['Enabled']) || $giftTeaser[$this->moduleName]['Enabled'] == 'no') { } else {
				...
				$cart_products = array($data['product_id']);
			...
			if ($eligible_gifts){
				...
				foreach ($eligible_gifts as $eligible_gift){
					$eligible_gift['description'] = $this->buildDescription($eligible_gift);	
					$eligible_gift['image'] = $this->model_tool_image->resize($eligible_gift['image'], 300, 300);
					$eligible_gift['url'] = $this->url->link('product/product', 'product_id=' . $eligible_gift['product_id']);
					
					$data['gifts'][] = $eligible_gift;
				}						
				
				if ($data['return_data']){
					return $data;
					} else {
					return $this->load->view('product/gifts.tpl', $data);
				}
