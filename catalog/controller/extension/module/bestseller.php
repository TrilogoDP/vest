...
$data['position'] = $setting['position'];
		...
		$results = $this->model_catalog_product->getBestSellerProducts($setting['limit'], true);//***

		//***mf begin
		/*$array_prod = array();
		$query_hide = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product_to_category WHERE category_id='62' OR category_id='64' OR category_id='99'");
		foreach ($query_hide->rows as $result) {
			//$array_prod[] = $result['product_id'];
		}*/
		//***mf end		
		
		if ($results) {
			foreach ($results as $result) {
				//if(in_array($result['product_id'], $array_prod)) continue;//***mf
				if ($result['image']) {
