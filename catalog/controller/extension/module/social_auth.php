<?php
    class ControllerExtensionModuleSocialAuth extends Controller {
        ...
        public function iframeGoogleLogin(){       
            $redirect_href = $this->url->link('extension/module/social_auth/iframeGoogleLogin');
        ...
        // https://www.facebook.com/privacy/consent/gdp/?params%5Bapp_id%5D=638784613888378&params%5Bauth_type%5D=%22rerequest%22&params%5Bkid_directed_site%5D=false&params%5Blogger_id%5D=%2242cbfbd9-6b44-43d5-80ec-3125e94572ea%22&params%5Bnext%5D=%22confirm%22&params%5Bredirect_uri%5D=%22https%3A%5C%2F%5C%2Fvest.in.ua%5C%2Ffb_login%5C%2F%22&params%5Breturn_scopes%5D=false&params%5Bscope%5D=%5B%22email%22%5D&params%5Bsteps%5D=%7B%7D&params%5Btp%5D=%22unspecified%22&params%5Bcui_gk%5D=%22%5BPASS%5D%3A%22&params%5Bis_limited_login_shim%5D=false&source=gdp_delegated
        public function iframeFacebookLogin() {
            $redirect_href = $this->url->link('extension/module/social_auth/iframeFacebookLogin');
            ...
            // Логирование текущего редиректа URI
            $this->log->write("Redirect URI logged: " . $redirect_href);
            
            // Log user-agent for debugging
            $user_agent = $_SERVER['HTTP_USER_AGENT'];
            $this->log->write("User-Agent: " . $user_agent);
            
            if (!isset($this->request->get['code'])) {
                // Логирование Facebook Login URL перед редиректом
                $this->log->write("Redirecting to Facebook login URL: " . $link);
                $this->response->redirect($link);
            } else {
                $code = $this->request->get['code'];
                $this->log->write("Received code: " . $code);
                ...
                $this->log->write("Token data: " . json_encode($data));
                
                $url = 'https://graph.facebook.com/me?fields=email,about,first_name,last_name&access_token=' . $data->access_token;
                
                $result = $this->getCurl($url);
                $this->log->write("User data: " . json_encode($result));
                
                if (isset($result->error)) {
                    $this->log->write("Error: " . $result->error->message);
                ...
                if (isset($result->id)) {
                    ...
                    $this->log->write("Return data: " . json_encode($return_data));
                    
                    if ($return_data['status']) {
                        $redirect = $return_data['redirect'];
                        $this->log->write("Setting output for redirect to: " . $redirect);
                        $this->response->setOutput('<script>
                        console.log("Redirecting to: '.$redirect.'");
                        if (window.opener) {
                            window.opener.document.location = "'.$redirect.'";
                            window.close();
                        } else {
