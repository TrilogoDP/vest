<?php
	...
	class ControllerExtensionModuleMicrodataPro extends Controller {
		...
		public function index() {
			...
			if(($route == "common/home" || $route == "product/product" || $route == "product/category" || $route == "product/manufacturer"  || $route == "product/manufacturer/info" || $route == "information/information" || $route == "product/special")&&$b){
				...
				if($route == "product/product" && isset($this->request->get['product_id']) && $this->request->get['product_id'] != 0 && $a){
					...
					if($product_info){
						...
						$this->data['microdata_manufacturer'] = !empty($product_info['manufacturer'])?$product_info['manufacturer']:'Vest';
						...
						if (@$this->config->get('config_microdata_sku')){
							$this->data['microdata_sku'] = $product_info['sku'];
							if (!$this->data['microdata_sku']){
								if ($this->isRealNumeric($product_info['model'])){
									$this->data['microdata_sku'] = trim($product_info['model']);
								}
							}
							if (!$this->data['microdata_sku']){
								$this->data['microdata_sku'] = trim($product_info['product_id']);
							}
						}
						
						if (@$this->config->get('config_microdata_mpn')){
							$this->data['microdata_mpn'] = $product_info['mpn'];
							
							if (!$this->data['microdata_mpn']){
								if ($this->isRealNumeric($product_info['model'])){
									$this->data['microdata_mpn'] = trim($product_info['model']);
								}
							}
							
							if (!$this->data['microdata_mpn']){
								$this->data['microdata_mpn'] = trim($product_info['product_id']);
							}
						}
						...
						if (!empty($this->request->get['option_id'])){
							...
							$query = $this->db->query("SELECT pov.*, ov.*, ovd.* FROM " . DB_PREFIX . "product_option_value pov
							LEFT JOIN " . DB_PREFIX . "option_value ov ON (pov.option_value_id = ov.option_value_id) 
							LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) 
							WHERE pov.product_id = '" . (int)$this->request->get['product_id'] . "' AND pov.product_option_value_id = '" . (int)$this->request->get['option_id'] . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY ov.sort_order LIMIT 1");
							...
							if ($query->num_rows && !empty($query->row['name'])) {
								$this->data['microdata_heading_title'] = $this->microdatapro->clear(@$product_info['name'] . ' ' . mb_strtolower($query->row['name']));        

								if (!empty($query->row['sku'])) {
									$this->data['microdata_sku'] = trim($query->row['sku']);
								} else {
									$this->data['microdata_sku'] = trim((int)$this->request->get['product_id'] . '-' . (int)$query->row['option_id'] . '-' . (int)$query->row['option_value_id']);
								}                               

								if (!empty($query->row['ean'])) {
									$this->data['microdata_ean'] = trim($query->row['ean']);
								}

								// Проверяем остаток опции
								if (isset($query->row['quantity']) && $query->row['quantity'] <= 0) {
									$this->data['microdata_stock'] = 0;
								} else {
									// Если остаток больше 0, устанавливаем фактическое значение
									$this->data['microdata_stock'] = $query->row['quantity'];
								}
							}
						}
				...
				if($route == "product/manufacturer"){
					$this->data['microdata_url'] = $this->url->link('product/manufacturer');	
					$this->data['breadcrumbs'] = $this->breadcrumbs('manufacturer');
				}
				...
				if($this->microdatapro->opencart_version(0) == 2 && $this->microdatapro->opencart_version(1) >= 2){ //over 2.2
					...
					if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/module/microdatapro/microdatapro.tpl')) {
						return $this->load->view($this->config->get('config_template') . '/template/extension/module/microdatapro/microdatapro.tpl', $this->data);
						} else {
						return $this->load->view('default/template/extension/module/microdatapro/microdatapro.tpl', $this->data);					
					}
					}else{ //1.X
					if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/module/microdatapro/microdatapro.tpl')) {
						$this->template = $this->config->get('config_template') . '/template/extension/module/microdatapro/microdatapro.tpl';
						} else {
						$this->template = 'default/template/extension/module/microdatapro/microdatapro.tpl';
					}
					return $this->render();
				}
		...
		public function tc_og($data = array()){
			...
			if($this->microdatapro->opencart_version(0) == 2 && $this->microdatapro->opencart_version(1) >= 2){ //over 2.2
				...
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'template/extension/module/microdatapro/tc_og.tpl')) {
					return $this->load->view($this->config->get('config_template') . 'template/extension/module/microdatapro/tc_og.tpl', $this->data);
					} else {
					return $this->load->view('default/template/extension/module/microdatapro/tc_og.tpl', $this->data);					
				}	
				}else{ //1.X
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'template/extension/module/microdatapro/tc_og.tpl')) {
					$this->template = $this->config->get('config_template') . 'template/extension/module/microdatapro/tc_og.tpl';
					} else {
					$this->template = 'default/template/extension/module/microdatapro/tc_og.tpl';
				}
				return $this->render();
			}
		} 	
		...
		public function min_max($results = array(0,0), $page_id = false, $page_type = false){
			...
			if($page_type == 'category'){
				...
				$review_data = $this->model_catalog_review->getReviewsByCategory($page_id);
				
				if ($review_data['total_rating']){
					$this->data['microdata_category_rating_count'] = $review_data['total_rating'];
				}
				
				if ($review_data['avg_rating']){
					$this->data['microdata_category_rating'] = $review_data['avg_rating'];
				}
				...
				$this->load->model('catalog/product');
				$price_data = $this->model_catalog_ocfilter->getProductPrices($filter_data);
				$this->data['total_products'] = $this->model_catalog_product->getTotalProducts($filter_data);
				
				$this->data['prices'] = array();
				foreach ($price_data as $__price){
					$this->data['prices'][] = $this->convert((float)($__price), $this->config->get('config_currency'), $this->data['microdata_code']);
				}
				
				$this->load->model('catalog/category');
				
				$category_info = $this->model_catalog_category->getCategory($page_id);
				if ($category_info) {
					if ($category_info['image']) {
						$this->data['wf_microdata_thumb'] = HTTPS_SERVER . 'image/' . $category_info['image'];
						} else {
						$this->data['wf_microdata_thumb'] = '';
					}
				}
			}
			
			if($page_type == 'manufacturer'){	
				$review_data = $this->model_catalog_review->getReviewsByManufacturer($page_id);
				...
				$this->load->model('catalog/product');
				$price_data = $this->model_catalog_review->getMinMaxPriceByManufacturer($page_id);
				$this->data['total_products'] = $this->model_catalog_product->getTotalProducts($filter_data);
			...
			if($page_type == 'special'){
				$review_data = $this->model_catalog_review->getReviewsBySpecial();
				...
				$this->load->model('catalog/product');
				$price_data = $this->model_catalog_review->getMinMaxPriceBySpecial();
				$this->data['total_products'] = $this->model_catalog_product->getTotalProductSpecials();
		...
		public function ___min_max($results = array(0,0), $page_id = false, $page_type = false){
			...
			if($page_type == 'category'){
				...
				$this->model_catalog_review->getReviewsByCategory($page_id);
			...
			if($this->microdatapro->opencart_version(0) == 2 && $this->microdatapro->opencart_version(1) >= 2){ //over 2.2
				...
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'template/extension/module/microdatapro/min_max.tpl')) {
					return $this->load->view($this->config->get('config_template') . 'template/extension/module/microdatapro/min_max.tpl', $this->data);
					} else {
					return $this->load->view('default/template/extension/module/microdatapro/min_max.tpl', $this->data);
				}	
				}else{ //1.X
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'template/extension/module/microdatapro/min_max.tpl')) {
					$this->template = $this->config->get('config_template') . 'template/extension/module/microdatapro/min_max.tpl';
					} else {
					$this->template = 'default/template/extension/module/microdatapro/min_max.tpl';
				}
				return $this->render();
			}
		} 
		...
		public function products($results = array(), $type = 'json', $related = false){
			...
			if($results){
				foreach ($results as $result) {	
					...
					$microdata_stock = ($this->config->get('config_product_in_stock'))?777:@(int)($result['quantity']?$result['quantity']:0);
					
					if (@$this->config->get('config_microdata_sku')){
						$microdata_sku = $result['sku'];
						if (!$microdata_sku){
							if ($this->isRealNumeric($result['model'])){
								$microdata_sku = trim($result['model']);
							}
						}
						if (!$microdata_sku){
							$microdata_sku = trim($result['product_id']);
						}
					}
					
					if (@$this->config->get('config_microdata_mpn')){
						$microdata_mpn = $result['mpn'];
						
						if (!$microdata_mpn){
							if ($this->isRealNumeric($result['model'])){
								$microdata_mpn = trim($result['model']);
							}
						}
						
						if (!$microdata_mpn){
							$microdata_mpn = trim($result['product_id']);
						}
					}
					
					$microdata_ean = @$this->config->get('config_microdata_ean')?$result['ean']:false;
					...
					'microdata_stock' => $microdata_stock,
					'mpn' => $this->microdatapro->clear(@$microdata_mpn),
					'ean' => $this->microdatapro->clear(@$microdata_ean),
					'sku' => $this->microdatapro->clear(@$microdata_sku),
					'model'        => $this->microdatapro->clear(@$result['model']),
					...
					'manufacturer'  => $this->microdatapro->clear($result['manufacturer']?$result['manufacturer']:'Vest'),
			...
			if($this->microdatapro->opencart_version(0) == 2 && $this->microdatapro->opencart_version(1) >= 2){ //over 2.2
				...
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'template/extension/module/microdatapro/products_'.$type.'.tpl')) {
					return $this->load->view($this->config->get('config_template') . 'template/extension/module/microdatapro/products_'.$type.'.tpl', $this->data);
					} else {
					return $this->load->view('default/template/extension/module/microdatapro/products_'.$type.'.tpl', $this->data);					
				}	
				}else{ //1.X
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'template/extension/module/microdatapro/products_'.$type.'.tpl')) {
					$this->template = $this->config->get('config_template') . 'template/extension/module/microdatapro/products_'.$type.'.tpl';
					} else {
					$this->template = 'default/template/extension/module/microdatapro/products_'.$type.'.tpl';
				}
				return $this->render();
			}
		}
		...
		public function breadcrumbs($page, $page_data = array()) {
			...
			if($this->microdatapro->opencart_version(0) == 2 && $this->microdatapro->opencart_version(1) >= 2){ //over 2.2
				...
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'template/extension/module/microdatapro/breadcrumbs.tpl')) {
					return $this->load->view($this->config->get('config_template') . 'template/extension/module/microdatapro/breadcrumbs.tpl', $this->data);
					} else {
					return $this->load->view('default/template/extension/module/microdatapro/breadcrumbs.tpl', $this->data);					
				}	
				}else{ //1.X
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'template/extension/module/microdatapro/breadcrumbs.tpl')) {
					$this->template = $this->config->get('config_template') . 'template/extension/module/microdatapro/breadcrumbs.tpl';
					} else {
					$this->template = 'default/template/extension/module/microdatapro/breadcrumbs.tpl';
				}
				return $this->render();
			}		
