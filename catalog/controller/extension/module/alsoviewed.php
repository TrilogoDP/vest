<?php
	class ControllerExtensionModuleAlsoViewed extends Controller {
		public function index() {
			...
			$this->load->language('extension/module/alsoviewed');
		...
		//	$data['position'] = $setting['position'];
			...
			foreach ($alsoViewedProducts as $product_info) {
				...
				return $this->load->view('module/alsoviewed.tpl', $data);
			...
			private function listalsoViewedById($product_id, $limit = 5) {
				...
				$data = $this->db->query("SELECT * FROM `" . DB_PREFIX . "alsoviewed` WHERE `low` = '" . (int)$product_id ."' OR `high` = '" . (int)$product_id ."' ORDER BY `number` DESC LIMIT " . (int)$limit ."");
