	...
	class ControllerExtensionAnalyticsFbMarketing extends Controller { 
		...
		public function index() {
			...
			if (!empty($status) && !empty($pixel_id) ) {
				...
				$fb_pixel_code = "<!-- TikTok Pixel Code Start -->
				<script>
				!function (w, d, t) {
		...
		private function constructEventString($event, $product_catalog_id) {
			...
			$event_string .= "ttq.track('" . $this->ttqpixel_events[$type] . "', {" . $options . "});";
		...
		protected function getCustomEventOptions($event_data, $event, $product_catalog_id) {
			...
			$contents = array();
			$tokens = array('[product_price]','[cart_total]','[order_total]');
			...
			switch ($event) {
				...
				if (!empty($order_id)) {
					...
					$options .= 'value: \'' . $this->formatValue((!in_array($event_data['value'],$tokens) ? $event_data['value'] : ($event_data['value'] == '[order_total]' ? $order_value : '0.00' ) )) . '\'';
				...
				$options .= 'value: \'' . $this->formatValue((!in_array($event_data['value'],$tokens) ? $event_data['value'] : ($event_data['value'] == '[cart_total]' ? $value : '0.00' ) )) . '\'';
		...
		protected function formatValue ($value) {
			...
			return $this->currency->format($value, $this->session->data['currency'], '', false);
