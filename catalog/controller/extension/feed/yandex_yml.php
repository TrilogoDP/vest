<?php
ini_set('max_execution_time', 300);//***mf
/**
...
//---- Config section ----
	...
	protected function generateYml() {
		if ($this->config->get($this->CONFIG_PREFIX.'status')) {
			...
			$supported_currencies = array('RUR', 'RUB', 'USD', 'BYR', 'KZT', 'EUR', 'UAN');
			...
			foreach ($currencies as $currency) {
				if ($currency['code'] != $offers_currency && $currency['status'] == 1) {
					$this->setCurrency(str_replace("UAN","UAH",$currency['code']), number_format(1/$this->currency->convert($currency['value'], $offers_currency, $shop_currency), 4, '.', ''));
				}
			}
			...
			$allowed_manufacturers = $this->config->get($this->CONFIG_PREFIX.'manufacturers');$allowed_manufacturers='';
...
//				$data['cbid'] = 15;
				...
				$data['vendor'] = str_replace("SYSTEMS SUNLIGHT S.A.","Sunlight",$product['manufacturer']);//*** GS
				$data['vendorCode'] = $product['model'];
				$data['vendorCode'] = $product['sku'];//***mf
...
//				$data['barcode'] = $product['sku'];
				...
				*/
				...
				//$data['description'] .= 'Более подробно на сайте: <a href="'.$href.'">'.$href.'</a>';
				*///***mf
				//---- Описание товара ----
	...
	*/
	 ...
	 * Валюты
	 *
	 * @param string $id - код валюты (RUR, RUB, USD, BYR, KZT, EUR, UAN)
	...
	protected function setCurrency($id, $rate = 'CBRF', $plus = 0) {
		$allow_id = array('RUR', 'RUB', 'USD', 'BYR', 'KZT', 'EUR', 'UAN');
