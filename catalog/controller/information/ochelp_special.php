<?php
	...
	class ControllerInformationOchelpSpecial extends Controller {
		...
		public function index() {
			...
			'text2' => $this->language->get('text_home2'),
		...
		public function info() {
			...
			if ($result) {
				...
				$this->document->setSCOgTitle($data['heading_title']);
				$this->document->setSCOgType('website');
			//	$this->document->setOpengraph('og:site_name', $this->config->get('config_name'));
				if ($data['banner']){
				//	$this->document->setSCOgImage('og:image', $data['banner']);
					$this->document->setSCOgImage($data['banner']);
					$this->document->setOgImage($data['banner']);
				}
				
				$this->document->setSCOgUrl($this->url->link('information/ochelp_special', 'special_id=' . $this->request->get['special_id']));
				$this->document->setSCOgDescription($result['description']);
				...
				if ($result['date_added'] != '0000-00-00' && date('Y-m-d', strtotime($result['date_added'])) > date('Y-m-d')){
					$data['special_date_diff'] = date('Y-m-d', strtotime($result['date_added']));
					$data['special_time_start_long'] = $this->language->get('special_time_start_long');
				}
				
				//	var_dump($data['special_date_diff']);
