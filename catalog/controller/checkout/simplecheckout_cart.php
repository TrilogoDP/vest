		...
		public function index() {
			...
			foreach ($products as $product) {
				...
				foreach ($product['option'] as $option) {
					...
					if ($option['image']) {
						$option_thumb = $this->model_tool_image->resize($option['image'], 47, 47);
					 }else {
						$option_thumb = false;
					}
				...
				if ($product['old_price']){
					...
					if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
						$old_total = $this->simplecheckout->formatCurrency($this->tax->calculate($product['old_price'], $product['tax_class_id'], $this->config->get('config_tax')) * $product['quantity']);
						} else {
						$old_total = false;
					}
				}
				...
				if ($version >= 200) {
                    ...
                    'thumb'     => $image,
                    'name'      => $product['name'],
					'gift_label'=> !empty($product['gift_label'])?$product['gift_label']:'',
                    'model'     => $product['model'],
                    ...
                    'total'     => $total,
					'old_total' => $old_total,
					'action_stickers' => $product['action_stickers'],
