<?php
	...
	class ControllerFeedGoogleReviewBuilder extends Controller {
		private $limit = 1000;
		private $langprefix = '';
		...
		public function cron(){
			...
			foreach ($languages as $language){									
				...
				$file = DIR_FEEDS . 'google_reviews_' . $language['code'] . '.xml';
				
				$this->echoLine('[i] Язык ' . $language['code'] . ', файл отзывов ' . $file);				
				...
				foreach ($reviews as $review){
					...
					if ($product){
						...
						$output .= '<review_url type="group">' . $this->link('product/product', 'product_id=' . $review['product_id']) . '</review_url>'  . PHP_EOL;
						...
						$output .= '<product_url><![CDATA[' .  $this->link('product/product', 'product_id=' . $review['product_id']) . ']]></product_url>' . PHP_EOL;
						$output .= '</product>' . PHP_EOL;
							...
							foreach ($options as $option){															
								...
								if (!empty($option['product_option_value'])){
									foreach ($option['product_option_value'] as $option_value){
										...
										$output .= '<product_url><![CDATA[' .  $this->link('product/product', 'product_id=' . $review['product_id'] . '&option_id=' . (int)$option_value['product_option_value_id']) . ']]></product_url>' . PHP_EOL;
										$output .= '</product>' . PHP_EOL;	
