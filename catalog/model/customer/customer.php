...
class ModelCustomerCustomer extends Model {
	...
	public function getCountOrdersByCustomer($data) {

		if ($data['email']){
			$sql = "SELECT COUNT(DISTINCT order_id) as total FROM `" . DB_PREFIX . "order` WHERE 
			(email LIKE ('" . $this->db->escape($data['email']) . "') OR telephone LIKE ('" . $this->db->escape($data['telephone']) . "')) 
			AND order_status_id > 0";
		} else {
			$sql = "SELECT COUNT(DISTINCT order_id) as total FROM `" . DB_PREFIX . "order` WHERE 
			telephone LIKE ('" . $this->db->escape($data['telephone']) . "')
			AND order_status_id > 0";
