<?php
	class ModelMarketingCoupon extends Model {
		...
		public function generateCouponForAFP($order_id){
			
			$order_id = (int)$order_id;
			
			$code = 'AFP-' . $order_id . '-' . mb_strtoupper(substr(md5($this->config->get('config_encryption') . $order_id), 0, 4));
			
			$coupon_data = array(
			'name' 			=> $code,
			'code' 			=> $code,
			'discount' 		=> 0,
			'discount_type' => 'afp_discount',
			'type'      	=> 'P',
			'total'     	=> 20,
			'logged'    	=> 0,
			'auto'      	=> 1,
			...
			//UPDATE AFP INFO
			$this->db->query("UPDATE `" . DB_PREFIX . "order_afp` SET afp_promocode = '" . $coupon_id . "' WHERE order_id = '" . $order_id . "'");
