<?php
	class ModelFeedGoogleMerchantCenter extends Model {
		public function getTaxonomy($category_id) {
			$query = $this->db->query("SELECT `".DB_PREFIX."feed_manager_taxonomy`.taxonomy_id, `".DB_PREFIX."feed_manager_taxonomy`.name 
			FROM `".DB_PREFIX."feed_manager_category` 
			RIGHT JOIN `".DB_PREFIX."feed_manager_taxonomy` ON `".DB_PREFIX."feed_manager_category`.taxonomy_id=`".DB_PREFIX."feed_manager_taxonomy`.taxonomy_id 
			WHERE `".DB_PREFIX."feed_manager_category`.category_id LIKE '".$category_id."' 
			OR `".DB_PREFIX."feed_manager_taxonomy`.status LIKE '1' ORDER BY `".DB_PREFIX."feed_manager_category`.category_id DESC LIMIT 1;");
			return $query->row;
		}
		
		public function getTaxonomyName($taxonomy_id) {
			$query = $this->db->query("SELECT name FROM `".DB_PREFIX."feed_manager_taxonomy` WHERE taxonomy_id = '" . (int)$taxonomy_id . "'");
		...
		public function getTaxonomyByCategoryExplicit($category_id) {
		...
		public function getReviews(){
			...
			$sql = "SELECT r.review_id, r.author, r.rating, r.text, p.product_id, pd.name, p.mpn, p.ean, p.model, p.sku, p.image, r.date_added ";
			...
			$sql .= "WHERE p.date_available <= NOW() AND r.rating >= 3 AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND p.status = '1' AND r.status = '1' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' GROUP BY r.review_id ORDER BY r.date_added DESC";
		...
		public function getProductsOptioned($language_id, $store_id){
			...
			AND p.quantity > 0
			AND p.product_id IN (SELECT DISTINCT product_id FROM " . DB_PREFIX . "product_option WHERE 1)";
		...
		public function getProduct($product_id, $lang, $store) {
			$product_data = $this->cache->get('product_fgmc.' . $product_id . '.' . $lang . '.' . $store);
			...
			if (!$product_data) {
				...
				if ($query->num_rows) {
					...
					$this->cache->set('product_fgmc.' . $product_id . '.' . $lang . '.' . $store, $product_data);
