<?php
	class ModelCatalogOCFilter extends Model {
		...
		// webfun
		public function webfunGetOCFilterOptionsBySort() {
			$options_data = $this->cache->get('ocfilter.webfun.shit.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . $this->config->get('config_customer_group_id'));
			...
			if (!$options_data){
				...
				$this->cache->set('ocfilter.webfun.shit.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . $this->config->get('config_customer_group_id'), $options_data);
		...
		public function getSliderRange($option_id, $data) {
			...
			$sql .= " WHERE p.status = '1' AND p.archive = '0' AND p2c.category_id = '" . (int)$data['filter_category_id'] . "' AND oov2p.value_id = '0' AND oov2p.option_id = '" . (int)$option_id . "'";
		...
		public function getCategoriesByManufacturerId($manufacturer_id, $nocache = false) {
			$cache_key = 'ocfilter.categories.manufacturer.' . (int)$manufacturer_id . '.' . (int)$this->config->get('config_store_id') . '.' . (int)$this->config->get('ocfilter_sub_category');
			
			$categories_data = $this->cache->get($cache_key);
			
			if ($nocache) $categories_data = false;
			
			if (false !== $categories_data) {
				return $categories_data;
			}
			
			$sql = "SELECT p2c.category_id FROM " . DB_PREFIX . "manufacturer m LEFT JOIN " . DB_PREFIX . "manufacturer_to_store m2s ON (m.manufacturer_id = m2s.manufacturer_id) LEFT JOIN " . DB_PREFIX . "product p ON (m.manufacturer_id = p.manufacturer_id) LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id)";			
			
			$sql .= " WHERE p.status = '1' AND p.archive = '0' AND m2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
			...
			$this->cache->set($cache_key, $categories_data);
			
			return $categories_data;
		}
		
		public function getManufacturersByCategoryId($category_id, $nocache = false) {
			...
			if ($nocache) $manufacturers_data = false;
		...
		public function getProductPrices($data) {
			...
			$sql .= " WHERE p.status = '1' AND p.archive = '0' AND p.price > '0' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND p.date_available <= '" . $this->db->escape(date('Y-m-d')) . "'";
			...
			if ($this->config->get('ocfilter_consider_special')) {
				...
				$sql .= " WHERE p.status = '1' AND p.archive = '0' AND p.price > '0' AND ps.price > '0' AND p.date_available <= '" . $this->db->escape(date('Y-m-d')) . "' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < '" . $this->db->escape(date('Y-m-d')) . "') AND (ps.date_end = '0000-00-00' OR ps.date_end > '" . $this->db->escape(date('Y-m-d')) . "'))";
			...
			if ($this->config->get('ocfilter_consider_discount')) {
				...
				$sql .= " WHERE p.status = '1' AND p.archive = '0' AND p.price > '0' AND p.date_available <= '" . $this->db->escape(date('Y-m-d')) . "' AND pd.price > '0' AND pd.quantity > '0' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND pd.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((pd.date_start = '0000-00-00' OR pd.date_start < '" . $this->db->escape(date('Y-m-d')) . "') AND (pd.date_end = '0000-00-00' OR pd.date_end > '" . $this->db->escape(date('Y-m-d')) . "'))";
			...
			if ($this->config->get('ocfilter_consider_option')) {
				...
				$sql .= " WHERE p.status = '1' AND p.archive = '0' AND p.date_available <= '" . $this->db->escape(date('Y-m-d')) . "' AND pov.price > '0' AND pov.quantity > '0' AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
		...
		private function getManufacturersCounterSQL($data) {
			...
			$sql .= " WHERE p.status = '1' AND p.archive = '0'";
		...
		private function getOptionValuesCounterSQL($data) {
			...
			$union['main'] .= " WHERE p.status = '1' AND p.archive = '0' AND oov2p.value_id > '0'";
			...
			if ($params) {
				...
				foreach ($params as $option_id => $values) {
					...
					$union[$option_id] .= " WHERE p.status = '1' AND p.archive = '0' AND oov2p.option_id = '" . (int)$option_id . "'";
