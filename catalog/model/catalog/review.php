...
class ModelCatalogReview extends Model {
	...
	public function getReviewsByCategory($category_id){
	
		$sql = "SELECT AVG(r.rating) AS avg_rating, COUNT(DISTINCT r.review_id) AS total_rating FROM " . DB_PREFIX . "review r WHERE r.product_id IN 
		(SELECT p.product_id FROM " . DB_PREFIX . "category_path cp LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (cp.category_id = p2c.category_id) LEFT JOIN " . DB_PREFIX . "product p ON (p2c.product_id = p.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND cp.path_id = '" . (int)$category_id . "')";
	...
	public function getMinMaxPriceByManufacturer($manufacturer_id){
		$sql = "SELECT MIN(p.price) as min, MAX(p.price) as max FROM " . DB_PREFIX . "product p WHERE p.manufacturer_id = '" . (int)$manufacturer_id . "' AND p.status = '1' AND p.date_available <= NOW()";
	...
	public function getReviewsByManufacturer($manufacturer_id){
	
		$sql = "SELECT AVG(r.rating) AS avg_rating, COUNT(DISTINCT r.review_id) AS total_rating FROM " . DB_PREFIX . "review r WHERE r.product_id IN 
		(SELECT p.product_id FROM " . DB_PREFIX . "product p WHERE p.manufacturer_id = '" . (int)$manufacturer_id . "' AND p.status = '1' AND p.date_available <= NOW())";
	...
	public function getReviewsBySpecial(){
	
		$sql = "SELECT AVG(r.rating) AS avg_rating, COUNT(DISTINCT r.review_id) AS total_rating FROM " . DB_PREFIX . "review r WHERE r.product_id IN 
		(SELECT DISTINCT ps.product_id FROM " . DB_PREFIX . "product_special ps LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.quantity > '0' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())))";
