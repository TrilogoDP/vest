<?php
	class ModelExtensionTotalCoupon extends Model {
		...
		private function afpCouponTotal($total){
		...
		public function getCoupon($code) {
			...
			if ($status) {
				...
				'discount_type' => $coupon_query->row['discount_type'],
			...
			if (isset($this->session->data['coupon'])) {
				...
				if ($coupon_info) {										
					...
					foreach ($products as $product) {
						...
						if ($coupon_info['discount_type'] == 'afp_discount'){
							$afp_percent = $this->model_catalog_product->getProductMainCategoryAFPDiscount((int)$product['product_id']);								
							$discount = $product['total'] / 100 * $afp_percent;
							$status = false;
						}
