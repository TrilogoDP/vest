...
class ModelExtensionModuleOctProductFilter extends Model {
    ...
    public function getFilterStandards($global_id, $global_type, $store_id, $customer_group_id, $language_id, $lead_time_status = false, $id_category = false) {//***mf
        $product_ids = $this->getProductIDs($global_id, $global_type, $store_id, $customer_group_id);
        ...
        if ($product_ids) {
            ...
            if (!$standards) {
                ...
                $query = $this->db->query("SELECT DISTINCT fpsd.filter_name, fpsd.filter_name_mod, fpsd.filter_id, fpsd.filter_group_id, fg.sort_order FROM " . DB_PREFIX . "oct_filter_product_standard fpsd LEFT JOIN " . DB_PREFIX . "filter_group fg ON (fpsd.filter_group_id = fg.filter_group_id) WHERE fpsd.product_id IN (" . implode(',', $product_ids) . ") AND fpsd.language_id = '" . (int) $language_id . "' GROUP BY fpsd.filter_name ORDER BY fg.sort_order ASC")->rows;
                ...
                foreach ($query as $value) {
                    $standards[] = array(
                        ...
                        'values' => $this->getFilterStandardValues($value['filter_name'], $global_id, $global_type, $store_id, $customer_group_id, $language_id, $id_category)//***
                    );
                }
    ...
    public function getFilterStandardValues($filter_name, $global_id, $global_type, $store_id, $customer_group_id, $language_id, $id_category = false) {
        $product_ids = $this->getProductIDs($global_id, $global_type, $store_id, $customer_group_id);

        if ($product_ids) {
            $query = $this->db->query("SELECT DISTINCT fpsd.filter_id, fpsd.filter_value_mod, fpsd.filter_value FROM " . DB_PREFIX . "oct_filter_product_standard fpsd WHERE fpsd.product_id IN (" . implode(',', $product_ids) . ") AND fpsd.language_id = '" . (int) $language_id . "' AND fpsd.filter_name = '" . $this->db->escape($filter_name) . "' GROUP BY fpsd.filter_value ORDER BY fpsd.sort_order, fpsd.filter_value ASC")->rows;

            $result = array();

            //***mf category filters begin
            $cat_filters_id = array();
            if($id_category != false && (int)$id_category > 0){
                $query_cf = $this->db->query("SELECT * FROM " . DB_PREFIX . "category_filter WHERE `category_id`='".(int)$id_category ."'")->rows;
                 foreach ($query_cf as $value) {
                        $cat_filters_id[] = (int)$value['filter_id'];
                 }
            }
            //***mf category filters end            
            
            foreach ($query as $value) {
                if ($id_category != false && in_array($value['filter_id'], $cat_filters_id) == false) continue;//***
                $result[] = array(
    ...
    public function getSeo($seo_url) {
        //***mf begin delete price path 
        $seo_url_arr = explode("/", $seo_url);
        $new_seo = '';
        foreach($seo_url_arr as  $path){
            if(strpos($path, "price") === false){
                $new_seo .= $path.'/';
            }
        }
        ...
        return $this->db->query("
          ...
          WHERE
            (pfsd.seo_url = '" . $this->db->escape($seo_url) . "' OR pfsd.seo_url = '" . $this->db->escape($new_seo) . "')
          AND
            pfs.status = '1'
          AND
            pfsd.language_id = '" . $lang_id . "'
        ")->row;
    }
