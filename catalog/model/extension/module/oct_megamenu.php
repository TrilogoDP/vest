...
class ModelExtensionModuleOctMegamenu extends Model {
    ...
    public function getMegamenuCategory($megamenu_id) {
        ...
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "oct_megamenu_category mm LEFT JOIN " . DB_PREFIX . "category c ON (mm.category_id = c.category_id) WHERE mm.megamenu_id = '" . (int) $megamenu_id . "' ORDER BY c.sort_order ASC");
