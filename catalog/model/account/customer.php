<?php
	class ModelAccountCustomer extends Model {
		public function addCustomer($data) {
			...
			$mail->setHtml($this->load->view('mail/registration', $data));
		...
		public function editPasswordByTelephone($telephone, $password) {
			...
			$telephone = phoneToNineDigits($telephone);

			$this->db->query("UPDATE " . DB_PREFIX . "customer SET salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($password)))) . "', code = '' WHERE LOWER(telephone) = '" . $this->db->escape(utf8_strtolower($telephone)) . "' OR telephone = '" . $this->db->escape(preg_replace("/\D+/", "", $telephone)) . "'  OR ((TRIM(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(telephone,' ',''), '+', ''), '-', ''), '(', ''), ')', '')) LIKE '%". $this->db->escape(trim($telephone)) ."' )
					OR (TRIM(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(fax,' ',''), '+', ''), '-', ''), '(', ''), ')', '')) LIKE '%". $this->db->escape(trim($telephone)) ."' ))");
		}
		...
		public function getCustomerByEmail($email) {
			...
			if (!$query->num_rows){
				...
				if ($phone = phoneToNineDigits($email)){
					
					$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE 
					((TRIM(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(telephone,' ',''), '+', ''), '-', ''), '(', ''), ')', '')) LIKE '%". $this->db->escape(trim($phone)) ."' )
					OR (TRIM(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(fax,' ',''), '+', ''), '-', ''), '(', ''), ')', '')) LIKE '%". $this->db->escape(trim($phone)) ."' ))
					AND status = '1'");
		...
		public function getTotalCustomersByTelephone($telephone) {
			...
			$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE LOWER(telephone) = '" . $this->db->escape(utf8_strtolower($telephone)) . "' OR telephone = '" . $this->db->escape(preg_replace("/\D+/", "", $telephone)) . "' OR ((TRIM(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(telephone,' ',''), '+', ''), '-', ''), '(', ''), ')', '')) LIKE '%". $this->db->escape(trim($telephone)) ."' )
					OR (TRIM(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(fax,' ',''), '+', ''), '-', ''), '(', ''), ')', '')) LIKE '%". $this->db->escape(trim($telephone)) ."' ))");
