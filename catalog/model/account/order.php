...
class ModelAccountOrder extends Model {
	...
	public function getOrders($start = 0, $limit = 20) {
		...
		$query = $this->db->query("SELECT o.order_id, o.firstname, o.lastname, os.name as status, o.date_added, o.total, o.currency_code, o.currency_value, o.track_no, o.shipping_method FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_status os ON (o.order_status_id = os.order_status_id) WHERE o.customer_id = '" . (int)$this->customer->getId() . "' AND o.order_status_id > '0' AND o.store_id = '" . (int)$this->config->get('config_store_id') . "' AND os.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY o.order_id DESC LIMIT " . (int)$start . "," . (int)$limit);

		return $query->rows;
	}
	
	public function canDoNotCall() {
		
		$query = $this->db->ncquery("SELECT COUNT(o.order_id) as total FROM `" . DB_PREFIX . "order` o WHERE o.customer_id = '" . (int)$this->customer->getId() . "' AND o.store_id = '" . (int)$this->config->get('config_store_id') . "' AND (o.order_status_id IN (" . implode(',', $this->config->get('config_donotcall_status')) . ") OR LENGTH(track_no) > 2)");
