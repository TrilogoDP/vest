<?php
    ...
    */
    ...
    class ModelToolSimpleApiMain extends Model {
        ...
        public function getCountries($filter = '') {
            ...
            foreach ($results as $result) {
                ...
                if ($this->config->get('config_language_id') == 1){
                    $name = $result['name_ru']?$result['name_ru']:$result['name'];
                    } else {
                    $name = $result['name'];
                } 
        ...
        public function checkPhoneForUniqueness($phone, $register) {
            $phone = trim($phone);
            
            if ((!$this->customer->isLogged() && $register && $phone) || ($this->customer->isLogged() && $phone != $this->customer->getTelephone() && preg_replace("/\D+/", "", $phone) != preg_replace("/\D+/", "", $this->customer->getTelephone()))) {
                $this->load->model('account/customer');
                return $this->model_account_customer->getTotalCustomersByTelephone($phone) > 0 ? false : true;
            }
