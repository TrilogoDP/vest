<?php
	...
	class ModelHobotixHoboFAQ extends Model {
		...
		public function checkOCFilterOption($option_id){
			$query = $this->db->query("SELECT option_id FROM `" . DB_PREFIX . "ocfilter_option` WHERE option_id = '" . (int)$option_id . "' AND faq = 1");
		...
		public function getCheapestProductsForCategory($category_id, $limit = 1){
			...
			if (!empty($this->request->get['filter_ocfilter'])) {
				...
				$ocfilter_product_sql = $this->model_catalog_ocfilter->getSearchSQL($this->request->get['filter_ocfilter']);
				} else {
			...
			$sql .= " AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY price ASC LIMIT " . (int)$limit . "";
		...
		public function getExpensiveProductsForCategory($category_id, $limit = 1){		
			...
			$sql .= " AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY price DESC LIMIT " . (int)$limit . "";
		...
		public function getMostPopularProductsForCategory($category_id, $limit = 1){		
			...
			$sql .= " AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.viewed DESC LIMIT " . (int)$limit . "";
		...
		public function getMostReviewsProductsForCategory($category_id, $limit = 1){	
			$sql = "SELECT r.product_id, COUNT(*) AS total
			...
			$sql .= " AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' GROUP BY r.product_id ORDER BY total DESC LIMIT " . (int)$limit . "";
		...
		public function getNewestProductsForCategory($category_id, $limit = 1){	
			...
			$sql .= " AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.date_added DESC LIMIT " . (int)$limit . "";
