<?php
	...
	class ModelExportExchange1c extends Model {
		...
		*/
		...
		private function tryToRewriteOptionSKU($option_sku, $real_product){
			$exploded_real_product_model = explode('-', $real_product['model']);
			$exploded_option_sku		 = explode('-', $option_sku);

			//
			if (count($exploded_real_product_model) == 2 && count($exploded_option_sku) == 2){
				if (($exploded_real_product_model[1] . $exploded_option_sku[0]) == $real_product['sku']){
					return ($exploded_real_product_model[1] . $option_sku);
				}
			}
		...
		private function getOrderProductOptionSKU($order_id, $order_product_id){		
			$query = $this->db->query("SELECT pov.sku FROM `" . DB_PREFIX . "order_option` oo LEFT JOIN
			`" . DB_PREFIX . "order_product` op ON (oo.order_product_id = op.order_product_id) LEFT JOIN
			`" . DB_PREFIX . "product_option_value` pov ON (op.product_id = pov.product_id AND oo.product_option_id = pov.product_option_id AND oo.product_option_value_id = pov.product_option_value_id)
			WHERE oo.order_product_id = '" . (int)$order_product_id . "' AND oo.order_id = '" . (int)$order_id . "' LIMIT 1");
			
			if ($query->num_rows) {
				$this->log("Найден sku: " . $query->row['sku'] . " в заказе '" . $order_id . "'",2);
				return $query->row['sku'];
			}
			
			$this->log("Не найден sku в заказе '" . $order_id . "'",2);
			return 0;
		}
		...
		*/
		private function setCustomerAddress($order, $mode = 'shipping', $return_addr = false) {
			...
			'Корпус'			=> 'building',
			'Квартира'			=> 'flat',
			'Населенный пункт'  => 'field20',
			'Район_simple'  	=> 'field21',
			'Улица_simple'  	=> 'field22',
			'ДомКВ_simple'  	=> 'field23',
			'ГородJustin_simple'  	=> 'cityjustin',
			'СкладJustin_simple'  	=> 'warehousejustin',
			);
		...
		*/
		...
		public function checkIfOrderHasProductsWithExcludedTaxClass($order_id){
			$this->load->model('sale/order');
			$order_products = $this->model_sale_order->getOrderProducts($order_id);

			foreach ($order_products as $product){
				$query = $this->query("SELECT tax_class_id FROM `" . DB_PREFIX . "product` WHERE `product_id` = " . (int)$product['product_id']);

				if ($query->row['tax_class_id'] == $this->config->get('config_excluded_tax_class_id')){
					echo 'Товар з НДС, passing' . PHP_EOL;
					return true;
				}
			}
		...
		*/
		public function queryOrdersExport() {
			...
			//QUEUE
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_queue` ORDER BY RAND() LIMIT 5");
			
			foreach ($query->rows as $row){
				
				$order = $this->model_sale_order->getOrder($row['order_id']);
				if ($order && !$this->checkIfOrderHasProductsWithExcludedTaxClass($row['order_id'])){					
					$orders_export[$row['order_id']] = $order['order_status_id'];
				}								
			}
		...
		public function queryOrdersExport2() {
			$this->load->model('sale/order');
		...
		public function getNPRef($name){
			
			$query = $this->db->query("SELECT ref FROM `" . DB_PREFIX . "city` WHERE name LIKE('" . $this->db->escape($name) . "') LIMIT 1");
		...
		*/
		public function queryOrders($orders_export = array()) {
			
			$this->log("~ЭКСПОРТ ЗАКАЗОВ В УЧЕТНУЮ СИСТЕМУ",2);
			
		//	$orders_export = array('131995' => '1');
			...
			if (count($orders_export)) {
				...
				foreach ($orders_export as $order_id => $order_status_id) {					
					...
					$simpleCustomInfo = $this->model_extension_module_simplecustom->getCustomFields('order', $order['order_id'], $order['language_code']);									
					$order = array_merge($simpleCustomInfo, $order);
					...
					,'Роль'        			=> 'Продавец'
					,'Сумма'       			=> $order['total']
					,'СтатусЗаказа'  		=> $order['order_status']
					,'СтатусЗаказаИд'  		=> $order['order_status_id']
					,'ЭтоОбратныйЗвонок'  	=> ($order['order_status_id']==18)?'Истина':'Ложь'
					,'НеПерезванивать' 		=> $order['donotcall']?'Истина':'Ложь'
					...
					if (!trim($order['shipping_address_1'])){
						$order['shipping_address_1'] = $this->setCustomerAddress($order, $mode = 'shipping', $return_addr = true);
						} else {
						$order['shipping_address_1'] = trim($order['shipping_address_1'] . $order['shipping_address_2']);
					}
					
					//ДОСТАВКА И ОПЛАТА
					$document['Документ' . $document_counter]['Доставка'] = array(
					'Ид' 			=> $order['shipping_code'],
					'Область' 		=> $order['shipping_country'],
					'Город' 		=> trim($order['shipping_zone'])?trim($order['shipping_zone']):trim($order['shipping_address_1']),
					'ОтделениеТК' 	=> $order['shipping_city'],
					'Реф'       	=> ($order['shipping_code'] == 'novaposhta.novaposhta')?$this->getNPRef($order['shipping_city']):'Ложь',
					'Адрес' 		=> $order['shipping_address_1'],
					'Способ' 		=> trim(preg_replace("/<img[^>]+\>/i", "", $order['shipping_method']))				
					);
					
					$document['Документ' . $document_counter]['Оплата'] = array(
					'Ид' 		=> $order['payment_code'],
					'ЭтоПредоплата' => (in_array($order['payment_code'], array('pb', 'wayforpay')))?'Истина':'Ложь',
					'Способ' 	=> trim(preg_replace("/<img[^>]+\>/i", "", $order['payment_method']))
					);
					
					//	var_dump($document['Документ' . $document_counter]);
					...
					foreach ($products as $product) {
						...
						//overload guid with options sku
						
						$product_guid = $this->getOrderProductOptionSKU($order['order_id'], $product['order_product_id']);

						$product_guid = $this->tryToRewriteOptionSKU($product_guid, $real_product);
					
						if (!$product_guid && trim($real_product['sku'])){
							$product_guid = trim($real_product['sku']);
						}
						...
						$order_product_options_data = $this->model_sale_order->getOrderOptions($order_id, $product['order_product_id']);
						if ($order_product_options_data){
							foreach ($order_product_options_data as $order_product_option_data){
								$product['name'] = trim($product['name']) . ', ' . $order_product_option_data['name'] . ':' . $order_product_option_data['value'];
							}
						}
						
						//А теперь надо проверить, если у товара есть опции, их больше одной, а в заказе их нет, то мы заебашим его в комментарий:)
						$options = $this->model_catalog_product->getProductOptions($product['product_id']);
						//если у товара есть опции, но в заказе они не указаны
						if ($options && !$order_product_options_data){
							$document['Документ' . $document_counter]['Комментарий'] .= '\nТовар: ' . $product['name'] . ', ' . $product['quantity'] . ' шт., ' . $this->currency->format($product['price'], $this->config->get('config_currency'));
							} else {
					...
					if ($order['donotcall']) {
						$document['Документ' . $document_counter]['Комментарий'] .= "\nПокупатель поставил галочку НЕ ПЕРЕЗВАНИВАТЬ МНЕ, ВСЕ ДАННЫЕ ВВЕДЕНЫ ВЕРНО";
					}
					
					$query = $this->query("SELECT `title` FROM `" . DB_PREFIX . "order_total` WHERE `order_id` = " . $order_id . " AND `code` = 'shipping'");
					if ($query->num_rows) {
					//	$document['Документ' . $document_counter]['Комментарий'] .= "\nДоставка: " . preg_replace("/<img[^>]+\>/i", "", $query->row['title']);
					}
					
					if ($this->config->get('novaposhta_min_total_for_free_delivery') > 0 && $this->config->get('novaposhta_min_total_for_free_delivery') < $order['total']){
						$query = $this->db->query( "SELECT COUNT(*) as count FROM `" . DB_PREFIX . "product_to_category` WHERE category_id IN (SELECT category_id FROM " . DB_PREFIX . "category WHERE free_delivery = 1) AND product_id IN (SELECT product_id FROM " . DB_PREFIX . "order_product WHERE order_id = " . (int)$order_id . ")");
											
						if ($query->row['count'] > 0){
							$document['Документ' . $document_counter]['БесплатнаяДоставка'] = 'Истина';
							$document['Документ' . $document_counter]['Комментарий'] .= "\nБесплатная доставка!";
						}
				...
				//	var_dump($document['Документ' . $document_counter]['Комментарий']);
					
					// Промокод в комментарий
					$query = $this->query("SELECT `title` FROM `" . DB_PREFIX . "order_total` WHERE `order_id` = " . $order_id . " AND `code` = 'coupon'");
					if ($query->num_rows) {
						$document['Документ' . $document_counter]['Комментарий'] .= "\nПромокод: " . preg_replace("/<img[^>]+\>/i", "", $query->row['title']);
					}
		...
		*/
		private function parseClassifierProductCategories($xml, $parent_id = 0, &$num_categories = 0) {		
		}
		...
		*/
		private function parseClassifierCategories($xml, $parent_id = 0, &$num_categories = 0) {
