	...
	$('div[data-type="standard"] .expanded.compatibility input[type=checkbox]').each(function() {
		...
		$('div[data-type="standard"] .expanded.models').each(function() {
			if($(this).find('div a span').html().indexOf(name_f) > 0){
				if(is_checked){
					console.log($(this).find('div a span').html() + ' - ' + name_f);
					$(this).addClass('show');
				}else{
...
function oct_start_filter(count_totals, render_products, increase_limit, page) {
      ...
      if (render_products) {
        if (json['products']) {
          ...
          product += '<div class="product-layout product-list col-sm-6 col-xs-6">';
            product += '<div class="product-thumb'+ class_quantity +'"'+' title="'+ value['name'] +'"'+'>';
              ...
              product += '<div>';
                product += '<div class="caption">';
				  if(value['sku'].length > 0){//***
					...
					product += '<p class="cat-model"><span></span></p>';
				  }//***
  ...
  setTimeout(show_f, 2000);//***mf
}

//***mf begin
$( window ).load(function() {
  //setTimeout(show_f, 500);
  //oct_start_filter(true, true, 0, 1);
});
//***mf end
//--></script>
