<div class="form-group <?php if ($required) { ?>required<?php } ?> row-<?php echo $id ?>">
    ...
    <div class="<?php echo $page == 'checkout' ? 'form_wrap' : 'col-sm-10' ?>">
      ...
      <?php } elseif ($type == 'radio') { ?>
        ...
        <?php foreach ($values as $info_id => $info) { ?>
          <div class="radio">
            <label><input type="radio" name="<?php echo $name ?>" id="<?php echo $id ?>_<?php echo $info_id ?>" value="<?php echo $info['id'] ?>" <?php echo $value == $info['id'] ? 'checked="checked"' : '' ?> <?php echo $reload ? 'data-onchange="reloadAll"' : 'data-reload-payment-form="true"'?>><span><?php echo $info['text'] ?></span></label>
          </div>
        <?php } ?>
        </div>
