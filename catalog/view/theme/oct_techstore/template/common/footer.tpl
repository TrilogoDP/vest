					...
					<ul class="footer-contacts-ul">
						...
						<?php if ($oct_techstore_cont_phones) { ?>
							<?php foreach($oct_techstore_cont_phones as $element) { ?>
								<li>
									<a href="tel:+<?php echo preg_replace('/\D/', '', $element); ?>" class="phoneclick" onclick="window.location.href='tel:+<?php echo preg_replace('/\D/', '', $element); ?>';return false;">
	...
	$('document').ready(function(){
		<?php if ($isMobile) { ?>
			...
			if (window.SpeechRecognition) {
				...
				$('#mainsearch').after('<button class="voice_search_btn" id="voice-search-btn" title="Пошук"><i class="fa fa-microphone"></i></button>');
				...
				function _recognitionEndHandler(e){
					...
					$('#voice-search-btn').removeClass('active');
					$('#mainsearch').removeClass('voice_input_active').attr("placeholder", "<?php echo $text_retranslate_35; ?>");
...
<div id="cookie-consent">
	<div id="cookie-inner">
		<div id="cookie-text">
			...
			<button id="accept"><?php echo $text_cookie_close; ?></button>
		</div>   
	</div>
</div>		
...
<?php if(false && isset($oct_information_bar_value) && !empty($oct_information_bar_value) && $oct_information_bar_value) { ?>
		<style>
			.d-flex.align-items-center.justify-content-between{display: flex;
		...
		<script>
			...
			$(function() {
				if (getCookie('<?php echo $oct_information_bar_value; ?>') == 'undefined') {
