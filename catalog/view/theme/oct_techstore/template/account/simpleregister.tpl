<?php if(isset($phone_error)){ ?>
	<script>
		$('#content').parent().before('<div id="pp_phone-error" class="alert alert-danger"><i class="fa fa-exclamation-circle"></i><?php echo $phone_error; ?><button type="button" class="close" data-dismiss="alert">&times;</button></div>');
		$('html, body').animate({scrollTop: 0},500);
		setTimeout(function(){
			$('#pp_phone-error').remove();
		},5000);
	</script>
<?php } ?>
<style type="text/css">
...
<?php if (!$ajax && !$popup && !$as_module) { ?>
    ...
    <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="simplepage_form">
        <div class="simpleregister" id="simpleregister">
            <p class="sub-title-smoll"><?php echo $text_account_already; ?></p>
		...
		<div class="btn-group-register">
			...
			<p class="sub-title-smoll"><?php echo $register_with_text ?></p>
			<div class="btn-group-register-wrap">
