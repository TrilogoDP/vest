<div class="simplecheckout-block" id="simplecheckout_customer" <?php echo $hide ? 'data-hide="true"' : '' ?> <?php echo $display_error && $has_error ? 'data-error="true"' : '' ?>>
	...
	<div class="simplecheckout-block-content">
		...
		<?php if (!$is_logged) { ?>
			<div class="btn-group-register text-center">
				<div class="checkout-heading-wrap">
					...
					<?php if ($display_header || $display_login) { ?>
						<div class="checkout-heading panel-heading">
							...
							<?php if ($display_login) { ?>
								<span class="checkout-heading-button" style="border-bottom:1px dashed #2b3743">
		...
		<?php if ($display_agreement_checkbox) { ?>
			...
			<div id="agreement_checkbox" class="simplecheckout-block-content text-right"style="margin-top:0px; float:none; margin-right:0px; padding-bottom:10px; padding-top:0px;display: flex;align-items: center;">
				<?php foreach ($text_agreements as $agreement_id => $text_agreement) { ?>
					<input type="checkbox" name="agreements[]" value="<?php echo $agreement_id ?>" checked="checked" />
					<label style="font-size:13px !important;text-align: left;line-height: 16px !important;"><?php echo $text_agreement; ?></label>
				<?php } ?>
			</div>	
