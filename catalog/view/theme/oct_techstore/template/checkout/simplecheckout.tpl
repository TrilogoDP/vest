<?php if (!$ajax && !$popup && !$as_module) { ?>
            ...
            <?php if (!$cart_empty) { ?>
                ...
                <?php if ($steps_count > 1 && $menu_type == '2') { ?>
					...
					<?php /* if ($display_agreement_checkbox) { ?>
						...
						<div id="agreement_checkbox" class="simplecheckout-block-content text-right" style="margin-top:0px; float:none; margin-right:0px; padding-bottom:10px; padding-top:0px;">
						<?php foreach ($text_agreements as $agreement_id => $text_agreement) { ?>
						<div class="pretty p-default p-bigger">
						<input type="checkbox" name="agreements[]" value="<?php echo $agreement_id ?>" <?php echo in_array($agreement_id, $agreements) ? 'checked="checked"' : '' ?>>
						<div class="state p-success control-label">
						<label style="font-size:14px;"><?php echo $text_agreement; ?></label>
						</div>
						</div>
						<?php } ?>
						</div>	
					...
					<div class="simplecheckout-button-block buttons" id="buttons">
						<div class="simplecheckout-button-right">							
							<?php if ($steps_count > 1) { ?>
								<a class="button btn-primary button_oc btn oct-button" data-onclick="nextStep" id="simplecheckout_button_next"><span><?php echo $button_next; ?></span></a>
							<?php } ?>
							<a class="button btn-primary button_oc btn oct-button" <?php echo $block_order ? 'disabled' : '' ?> data-onclick="createOrder" id="simplecheckout_button_confirm"><span><?php echo $button_order; ?></span></a>
						</div>
                ...
                <?php if (!$popup && !$as_module) { ?>
                    <div class="simplecheckout-button-block buttons">
                        <div class="simplecheckout-button-right right"><a href="<?php echo $continue; ?>" class="button btn-primary button_oc btn oct-button"><span><?php echo $button_continue; ?></span></a></div>
					</div>
				<?php } ?>
			<?php } ?>
		</div>
	...
	<script>
		$(document).ready(function(){
			...
			<?php 
				$dataLayerFocusOutFields = array(
				'#customer_telephone' => '3',
				...
				$dataRadioChangeFields = array(
				'shipping_method' 	  => '8',
				'payment_method' 	  => '9',
				);	
				
				$dataSelectChangeFields = array(
				'#shipping_address_country_id' 	  	=> '10',
				'#shipping_address_zone_id'		  	=> '11',
				'#shipping_address_cityjustin' 	  	=> '11',
				'#shipping_address_warehousejustin'	=> '12',
				'#shipping_address_city'			=> '12'
				);	
			?>
			
			<?php foreach ($dataLayerFocusOutFields as $key => $value) { ?>
				$('<?php echo $key; ?>').on('focusout', function(){
					passEcommerceCheckoutOptionsToDataLayer('<?php echo $value; ?>', $('<?php echo $key; ?>').val());				
				});
			<?php } ?>
			
			<?php foreach ($dataRadioChangeFields as $key => $value) { ?>
				$('input[name=\'<?php echo $key; ?>\']').on('change', function(){
					passEcommerceCheckoutOptionsToDataLayer('<?php echo $value; ?>', $('input[name=\'<?php echo $key; ?>\']:checked').val());				
				});
			<?php } ?>
			
			<?php foreach ($dataSelectChangeFields as $key => $value) { ?>
				$('body').on('change', '<?php echo $key; ?>', function(){
					passEcommerceCheckoutOptionsToDataLayer('<?php echo $value; ?>', $('<?php echo $key; ?> option:selected').text());				
				});
			<?php } ?>
		});
	</script>
