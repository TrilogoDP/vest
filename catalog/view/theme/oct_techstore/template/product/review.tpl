<?php if ($reviews) { ?>
  ...
  <table class="table table-striped table-bordered review-table" itemprop="review" itemscope itemtype="http://schema.org/Review">
    <tr>
      <td style="width: 50%;">
        <span class="author" itemprop="author" itemscope itemtype="http://schema.org/Person"><span itemprop="name" class="name"><?php echo $review['author']; ?></span></span> <span class="oct-review-date"><?php echo $review['date_added']; ?></span><?php
        if (strripos($review['date_added'], '.') !== false) {
          $parts = explode('.', $review['date_added']);
          $meta_date_added = $parts[2] . '-' . $parts[1] . '-' . $parts[0];
          echo '<meta itemprop="datePublished" content="' . $meta_date_added . '">';
        }
        ?>
