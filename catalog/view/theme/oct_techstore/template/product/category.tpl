...
<div class="container wrap fdc categories_page content-row" style="content-visibility: auto">
	<input type="text" id="category_id" value="<?php echo $category_id; ?>" hidden />
	...
	<div class="row">
		<div class="col-sm-12">
			<h1 class="cat-header title_module"><?php echo $heading_title; ?><?php if (!empty($seo_page)) { echo $seo_page; } ?></h1>
		</div>
	</div>
	...
	<div class="row categories-wrap">
		...
		<div id="content" class="<?php echo $class; ?>">
			...
			<?php if ($categories && isset($oct_techstore_data['cat_show_subcat']) && $oct_techstore_data['cat_show_subcat'] == 'on') { ?>
				<h3 class="sub-title subcat-header"><?php echo $oct_choose_subcategory; ?></h3>
				<div  id="subcats" class="subcats-wrap subcats-row">
					<?php foreach ($categories as $category) { ?>
						<div class="item">
							<a href="<?php echo $category['href']; ?>" title="<?php echo $category['name']; ?>">
								<img class="img-responsive" src="<?php echo $category['thumb']; ?>" width="100px" height="100px" alt="<?php echo $category['name']; ?>" />
				...
				<div id="res-products">
					<div class="row">
						<?php foreach ($products as $product) { ?>
							<div class="product-layout product-item product-grid product-layout <?php echo (isset($product['product_preorder_status']) && $product['product_preorder_status'] != 1 && $product['quantity'] <= 0) ? ' no_quantity' : ''?>" title="<?php echo $product['name'] ?>" data-product-id="<?php echo $product['product_id']; ?>" 
									data-gtm-product='{<?php foreach ($product['ecommerceData'] as $ecommerceKey => $ecommerceValue) { ?>"<?php echo $ecommerceKey; ?>": "<?php echo $ecommerceValue; ?>",<?php } ?> "url": "<?php echo $product['href']; ?>"}'>
								<div class="image">
									...
									<?php if (isset($oct_lazyload) && $oct_lazyload) { ?>
										<a href="<?php echo $product['href']; ?>" class="lazy_link">
											<img data-original="<?php echo $product['thumb']; ?>" width="100%" height="258" src="<?php echo $oct_lazyload_image; ?>" class="img-responsive lazy" alt="<?php echo $product['name'] ?>" />
										</a>
										<?php } else { ?>
								...
								<div class="name <?php if ($product['rating'] === 0) { ?>no_rating<?php } ?>" title="<?php echo $product['subcat_sort_order']; ?> - <?php echo $product['sort_order'] ?>" alt="<?php echo $product['subcat_sort_order']; ?> - <?php echo $product['sort_order'] ?>">
									<a href="<?php echo $product['href']; ?>" title="<?php echo $product['subcat_sort_order']; ?> - <?php echo $product['sort_order'] ?>" alt="<?php echo $product['subcat_sort_order']; ?> - <?php echo $product['sort_order'] ?>"><?php echo $product['name'] ?></a>
								</div>
					...
					<div class="row webfun_row_load_more_product">
						
						<?php
							if (!empty($webfun_view_load_more)) {
								echo '<div class="1 webfun_load_more_product" data-limit="2">' . $text_webfun_load_more_product . '</div>';
							} else {
								echo '<div class="2 webfun_load_more_product" data-limit="2" style="display: none;">' . $text_webfun_load_more_product . '</div>';
							}
						?>
					</div>
...
<?php if ($description) { ?>
	<div id="description-col" class="cat-desc-box bg-light-blue" >
		<div class="wrap fdc">
			<div class="content">
				<?php if ($thumb) { ?>
			...
			if (!sessionStorage.alreadyClicked) {
				setTimeout(function(){					
					if(weekday !== 0) {
						if(isHourRange(9, 20)) {
							setTimeout(function(){
								$('.tooltip_mobile_call_back').show();
							},55000);
							
							$('#mobile_call_beack_btn').on('click', function(){
								$('.tooltip_mobile_call_back').hide();
							});
						} 
					} 					
				},5000);	
	...
	$(window).load(function() {}), $(document).ready(function() {
		window.onpopstate = function(t) {
			if (typeof t.path !== "undefined"){							 			
				console.log('onpopstate reload triggered'), t.path[0].location.href && ($(".ocfilter").append('<div class="webfun_ocfilter_preloader"></div>'), $("#res-products").eq(0).append('<div class="webfun_ocfilter_preloader"></div>'), location.href = t.path[0].location.href)
				} else {
				document.location && ($(".ocfilter").append('<div class="webfun_ocfilter_preloader"></div>'), $("#res-products").eq(0).append('<div class="webfun_ocfilter_preloader"></div>'), location.href = document.location)
			}
		}	
	});
</script>
...
<script src="catalog/view/theme/oct_techstore/js/collapse.js"></script>	

  <script>
    $(document).ready(function() {
      // Вибираємо елементи за вказаним шаблоном
      $('#subcats .item:nth-child(4n+1), #subcats .item:nth-child(4n+4)').addClass('selected');
    });
  </script>
<?php echo $footer; ?>						