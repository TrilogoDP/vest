<?php
	class Mail {
		...
		private function validateEmail(){
			...
			if (mb_strlen($this->to) < 5){
				return false;
			}
			
			if ($this->to == 'dummy-email-simple@localhost.net'){
				return false;
			}
			
			if (strpos('@localhost.net', $this->to)){
				return false;
			}
		...
		public function send($add_to_queue = true) {
			...
			if ($add_to_queue){
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "email_queue SET data = '" . $this->db->escape(base64_encode(json_encode($json))) . "'");
				return true;
			}
			...
			if ($this->protocol == 'mail') {
				...
				if (!$handle) {
					throw new \Exception('Error: Socket can not be created: ' . $errstr . ' (' . $errno . ')');
					} else {
