...
class XML2Array
{
    ...
    public static function createArray($input_xml)
    {
        ...
        if (is_string($input_xml)) {
            ...
            libxml_use_internal_errors(true); // Включаем внутреннюю обработку ошибок libxml

            try {
                $xml->loadXML($input_xml, LIBXML_PARSEHUGE);
                if (!is_object($xml) || empty($xml->documentElement)) {
                    throw new Exception();
                }
            } catch (Exception $ex) {
                $error = libxml_get_last_error(); // Получаем последнюю ошибку libxml
                if ($error) {
                    $errorMessage = '[XML2Array] Error parsing the XML string at line ' . $error->line . ': ' . PHP_EOL . $error->message;
                    libxml_clear_errors(); // Очищаем ошибки после обработки
                } else {
                    $errorMessage = '[XML2Array] Error parsing the XML string.' . PHP_EOL . $ex->getMessage();
                }
