	...
	public function agooconstruct($settings = array()) {
        ...
        */
    	...
    	if (isset($this->jetcache_settings['cache_mobile_detect']) && $this->jetcache_settings['cache_mobile_detect']) {
			...
			if (!class_exists('jc_Mobile_Detect')) {
		...
		if (!$this->config->get('blog_work') || (isset($pieces_array[0]) && $pieces_array[0] != 'blog') || !$this->file_cache) {
			...
			if (isset($pieces_array[0]) && $pieces_array[0] == 'blog') {
				...
				if ($pieces != 'db') {
					...
					$file_cache = DIR_CACHE . 'seocms'. $pieces_devider . $pieces . $folder_hache_devider . $hache_array[2] . $folder_hache_cache_devider . $folder_hache . '/index.html';
					...
					$this->dir_cache = DIR_CACHE . 'seocms/' . $pieces . $pieces_devider . $hache_array[2] . $folder_hache_devider . $folder_hache . $folder_hache_cache_devider;
