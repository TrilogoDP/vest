...
$fh = fopen(__FILE__, 'r');
if (!flock($fh, LOCK_EX | LOCK_NB)) { echo 'CAN NOT LOCK'; exit; }
...
$session = new Session();
//$session->start('default', 'cronssessionconstant1value');
...
/**************************************************************************/
	...
	foreach($query->rows as $row) {		
		...
		$period = 1; // pause 240 minutes  = 4 hours

		echo 'Задача ' . $row['pr_name'] . PHP_EOL;

		if (($mk-$row['cron_status'])/60 < $period or !$row['on_off']){
			echo date('Y-m-d H:i:s') . ' Пропускаем задачу по периоду ' . $row['pr_name'] . PHP_EOL;
			continue;
		} else {
			 echo date('Y-m-d H:i:s') . ' Начинаем задачу ' . $row['pr_name'] . PHP_EOL;
		}		
		...
		echo date('Y-m-d H:i:s') . ' >> Сейчас ' . $h . ' часов, выполнение стоит на ' . $row['text'] . PHP_EOL;
		echo date('Y-m-d H:i:s') . ' >> Сейчас ' . $d . ', день недели выполнение стоит на ' . $row['text1'] . PHP_EOL;
		echo date('Y-m-d H:i:s') . ' >> Сейчас ' . $w . ', день недели выполнение стоит на ' . $row['text2'] . PHP_EOL;
