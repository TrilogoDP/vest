...
<?php } ?>
                    ...
                    <div class="form-group">
                        <label class="col-sm-2 control-label" for="input-hidefilter">Прятать в фильтре</label>
                        <div class="col-sm-10">
                            <select name="hidefilter" id="input-hidefilter" class="form-control">
                                <?php if ($hidefilter) { ?>
                                    <option value="1" selected="selected"><?php echo $text_enabled; ?></option>
                                    <option value="0"><?php echo $text_disabled; ?></option>
                                    <?php } else { ?>
                                ...
                                <table id="images<?php echo $language['language_id']; ?>" class="table table-striped table-bordered table-hover">
                                    <thead>
                                        <tr>
                                            ...
                                            <td class="text-right">Категория</td>
                                                ...
                                                <tr id="image-row<?php echo $image_row; ?>">
                                                    <td class="text-left"><input type="text" name="banner_image[<?php echo $language['language_id']; ?>][<?php echo $image_row; ?>][title]" value="<?php echo $banner_image['title']; ?>" placeholder="<?php echo $entry_title; ?>" class="form-control" />
                                                        ...
                                                        <td class="text-right" style="width: 10%;">
                                                            <select name="banner_image[<?php echo $language['language_id']; ?>][<?php echo $image_row; ?>][category_id]" style="max-width:250px;">
                                                                <option value=""> -- Без категорії -- </option>
                                                                <?php foreach($categories as $category) { ?>
                                                                    <option value="<?php echo $category['category_id']; ?>" <?php echo ((int)$category['category_id'] == (int) $banner_image['category_id'])?'selected':'' ?>><?php echo $category['name']; ?></option>
                                                                <?php } ?>
                                                            </select>
                                                        </td>
                                                        <!--//***mf end -->                    
