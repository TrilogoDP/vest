		...
		success: function(json) {
			if ($('#product_update').prop('checked') && product_id == 0) {
				...
				if (json['success']) {
					...
					<!--<?php if ($option['column_options'] || true) { ?>-->
					if (link_ == 'option') { getLinkToColumn(product_id, 'product_option'); editOptionsMf($('#form-' + link_ + product_id).serializeArray()); };//***mf
					<!--<?php } ?>-->
...
function editProductPrice(product_id, value){

	mkf = $.ajax({type:'POST', async:false, dataType:'html', data:'product_id=' + product_id + '&price=' + value, url:'index.php?route=batch_editor/mikrof/editPrice&token=' + token,
		error: function() {
			alert('<?php echo $error_server; ?> Custom update');
		},			
		success: function(json) {
			$('#product-price-date-' + product_id).load('index.php?route=batch_editor/mikrof/getPriceDate&product_id='+ product_id +'&token=' + token);
		}
	});
...
function editProductDnup(product_id, value){

	mkf = $.ajax({type:'POST', async:false, dataType:'html', data:'product_id=' + product_id + '&dnup=' + value, url:'index.php?route=batch_editor/mikrof/editDnup&token=' + token,
		error: function() {
			alert('<?php echo $error_server; ?> Custom update');
		},			
		success: function(json) {
			$('#product-dnup-date-' + product_id).load('index.php?route=batch_editor/mikrof/getDnupDate&product_id='+ product_id +'&token=' + token);
		}
	});
}


function editProductMf(product_id) {
	mkf = $.ajax({type:'POST', dataType:'html', data:'product_id=' + product_id, url:'index.php?route=batch_editor/mikrof/edit&token=' + token,
		error: function() {
			alert('<?php echo $error_server; ?> Custom update');
		},			
		success: function(json) {
...
function editOptionsMf(dataIn) {//
	var jsonString = JSON.stringify(dataIn);
	mkf = $.ajax({type:'POST', data:{data : jsonString}, url:'index.php?route=batch_editor/mikrof/edit_option&token=' + token,
		error: function() {
			alert('<?php echo $error_server; ?> Custom update');
		},			
		success: function(json) {
	...
	$('#product_box').delegate('#product tbody input, #product tbody textarea, #product tbody select', 'blur', function(e) {
		...
		if (text != text_edit) {
			...
			xhr = $.ajax({type:'POST', dataType:'json', data:'selected[]=' + product_id + '&product_' + field + '=' + value + '&field=' + field + '&language_id=' + getLanguageId(), url:'index.php?route=batch_editor/index/editProduct&token=' + token,
				beforeSend: function(){
					if (field == 'price'){
						editProductPrice(product_id, value);
					}
				},
				error: function() {
					quick_edit = 0;
