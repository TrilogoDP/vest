	...
	public function index($refresh = false) {
		...
		if (isset($this->request->post['exchange1c_order_date'])) {
			...
			if ($this->config->get('exchange1c_order_date')) {		
				$data['order_date_export'] = date('Y-m-d H:i:s', strtotime($this->config->get('exchange1c_order_date')));
				$this->log($data['order_date_export'], 2);
			} else {
				$data['order_date_export'] = date('Y-m-d H:i:s', strtotime('2000-01-01 00:00:00'));
				$this->log($data['order_date_export'], 2);
			}
		}
