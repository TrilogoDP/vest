<?php
	...
	class ControllerExtensionModuleOctPopupCallPhone extends Controller {
		...
		public function history_standalone() {
			...
			$this->response->setOutput($this->load->view('extension/module/oct_popup_call_phone_history_standalone.tpl', $data));
		...
		public function history() {
			...
			foreach ($results as $result) {
				...
				$name = $telephone = $comment = $referer = '';
				
				foreach ($fields as $field) {
					//Это адская хрень исключительно для того, чтоб создать совместимость с ленью ребят из octemplates
					if (in_array($field['name'], array('name', 'Имя', "Введіть ваше ім'я"))){
						$name = $field['value'];
						continue;
					}				
					
					if (in_array($field['name'], array('telephone', 'Телефон', 'Введіть Ваш телефон'))){
						$telephone = $field['value'];
						continue;
					}
					
					if (in_array($field['name'], array('comment', 'Комментарий', 'Коментар'))){
						$comment = $field['value'];
						continue;
					}
					
					if (in_array($field['name'], array('referer'))){
						$referer = $field['value'];
						continue;
					}
				/*	
				...
				*/
				...
				'comment'  	=> $comment,
				'referer'  	=> $referer,
				'binotel_call_id'  	=> $result['binotel_call_id'],
