<?php
	class ControllerExtensionDashboardActivity extends Controller {
		...
		public function getstatusajax(){
			...
			if ($status == 'idle'){
				$this->response->setOutput('Ожидание');
			} else {
				$this->response->setOutput('Работаем, ' . $status);
			}		
		}
		
		public function setcommandajax(){
		...
		public function dashboard() {
			...
			$data['update_url'] = $this->url->link('extension/dashboard/activity/getstatusajax', 'token=' . $this->session->data['token'], true);
			$data['command_url'] = $this->url->link('extension/dashboard/activity/setcommandajax', 'token=' . $this->session->data['token'], true);
			...
			if ($handle = opendir(DIR_FEEDS)) {
				while (false !== ($file = readdir($handle))) {
					if ($file != "." && $file != "..") {
						$files[$file] = date('Y-m-d H:i:s', filemtime(DIR_FEEDS . $file));
					}
				}
				closedir($handle);
			}
			...
			return $this->load->view('extension/dashboard/feedupdater', $data);
