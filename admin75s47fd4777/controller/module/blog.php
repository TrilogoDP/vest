...
class ControllerModuleBlog extends Controller
{
	...
	public function index()
	{
        ...
        $text_fgcont = $this->language->get('text_fgcont');
        ...
        $text_strconc = $this->language->get('text_strconc');
        ...
        if ($date_diff > 7) {
            $ver_content = false;
			$text_opt = array( $text_ptth => array($text_dohtem =>$text_tsop, $redaeh =>$text_redaeh_stpo, $tnetnoc => $yreuq_dliub_ptth(array($text_ctlg=>$value_ctlg,$text_gnal=>$value_gnal,$text_rdda =>$value_rdda, $text_liame => $this->data['liame'], $text_rev=>$this->data['blog_version'], $text_rev_model=>$this->data['blog_version_model'] ))));
		    $context = $text_strconc($text_opt);
            $exceptionizer = new PHP_Exceptionizer(E_ALL);
		    try { $ver_content = htmlspecialchars(strip_tags(html_entity_decode($text_fgcont($rev_knil, FALSE , $context), ENT_QUOTES, 'UTF-8')), ENT_COMPAT, 'UTF-8'); $this->model_setting_setting->editSetting('asc_cnt', Array('asc_cnt_cnt'=>md5($rev_knil))); }  catch (E_WARNING $e) {
		...
		if ($this->data['blog_version'] != $blog_version || $this->config->get('ascp_install') === false) {
			...
			$this->data['text_blog_update'] = $this->language->get('text_update');
...
/***************************************/
	...
	public function schemes() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            /*
			if (SC_VERSION < 20) {
				$this->redirect($this->url->link('module/blog/schemes', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']]));
			} else {
            	$this->response->redirect($this->url->link('module/blog/schemes', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']]));
            }*/
		}
