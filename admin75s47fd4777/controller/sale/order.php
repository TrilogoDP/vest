<?php
	...
	protected function checkPermission() {
        ...
        'optsku'  => (isset($option['optsku'])) ? $option['optsku'] : '',
        'stock'  => (isset($option['stock'])) ? $option['stock'] : '',
        'supplier'  => (isset($option['supplier'])) ? $option['supplier'] : '',
		    'ean'  => (isset($option['ean'])) ? $option['ean'] : '',
        ...
        // oct_advanced_options_settings end
					...
					'name'    	 	   => $product['name'],
					'tax_class_id'     			=> $real_product['tax_class_id'],
					'excluded_tax_class_id'     => ($real_product['tax_class_id'] == $this->config->get('config_excluded_tax_class_id')),
				...
				'system_num'    => $result['system_num'],	
				'customer'      => $result['customer'],
				'customer_id'   => $result['customer_id'],
				'customer_href' => $result['customer_id']?$this->url->link('customer/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['customer_id'] , true):false,
        ...
        $data['text_oct_option_sku'] = $this->language->get('text_oct_option_sku');
		 $data['text_oct_option_ean'] = $this->language->get('text_oct_option_ean');
        ...
        // oct_advanced_options_settings end
				...
				foreach ($products as $product) {
					...
					foreach ($options as $option) {
						...
						// $option_name_query = $this->db->query("SELECT ov.name FROM " . DB_PREFIX . "option_value_description  AS ov LEFT JOIN " . DB_PREFIX . "order_option AS oo ON (ov.option_value_id =  oo.product_option_value_id) WHERE ov.language_id = '3' AND oo.product_option_value_id = '" . (int)$option['product_option_value_id'] . "' LIMIT 1");

						// $option['product_option_value_id'] = 112160;
						...
						// var_dump($option_name_query->row['name']);
						// echo '<br>';
						// var_dump((int)$option['product_option_value_id']);
	...
	public function zone() {
		...
		public function addOrderStatus() {		
			if(isset($_POST['order_id']) && isset($_POST['status_id']) && $_POST['order_id'] > 0 && $_POST['status_id'] > 0){
				$query = $this->db->query("INSERT INTO `" . DB_PREFIX . "order_history` SET `order_id` = '".$_POST['order_id']."', `order_status_id` = '".$_POST['status_id']."', `notify` = '0', `comment` = '', `date_added` = '".date("Y-m-d H:i:s")."'");
				$query = $this->db->query("UPDATE `" . DB_PREFIX . "order` SET `order_status_id` = '".$_POST['status_id']."' WHERE `order_id` = '".$_POST['order_id']."'");
			}
			return true;
		}
		//***mf end
