<?php
	...
	public function quick_update() {
		...
		public function getCSV(){
			...
			header("Content-Disposition: attachment; filename=vest_google_customer_".date('Y_m_d').".csv");
			...
			fputs($file, implode(',', $header) . PHP_EOL);
			
			$query = $this->db->query("SELECT DISTINCT(email), firstname, lastname, telephone FROM oc_customer WHERE email LIKE ('%@%')");
			
			foreach ($query->rows as $row){
				$line = array($this->removeNewLines($row['email']), $this->removeNewLines($row['firstname']), $this->removeNewLines($row['lastname']), 'UA', '', $this->removeNewLines($row['telephone']));
				fputs($file, implode(',', $line) . PHP_EOL);
			}
			
			$query = $this->db->query("SELECT DISTINCT(email), firstname, lastname, telephone FROM oc_order WHERE email LIKE ('%@%')");
		...
		if ($this->config->get('aqe_status') && $this->config->get('aqe_customer_customers_status')) {
			...
			$data['csv'] = $this->url->link('customer/customer/getCSV', 'token=' . $this->session->data['token'] . $url, true);
