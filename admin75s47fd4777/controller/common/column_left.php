<?php
	class ControllerCommonColumnLeft extends Controller {
        ...
        $oct_techstore[] = array(
         ...
         $oct_techstore[] = array(
          'name'     => $this->language->get('oct_information_bar'),
        ...
        $data['menus'][] = array(
          ...
          'children' => $oct_techstore
			...
			if ($this->user->hasPermission('access', 'batch_editor/index')) {
				...
				if ($this->user->hasPermission('access', 'report/product_deletedsku')) {
					$catalog[] = array(
					'name'	   => 'Удаленные SKU',
					'href'     => $this->url->link('report/product_deletedsku', 'token=' . $this->session->data['token'], true),
					'children' => array()		
					);
				}
      ...
      // OCFilter start
		  ...
		  // OCFilter end
				...
				*/
				...
				if ($this->user->hasPermission('access', 'tool/oct_modification_manager')) {
					$extension[] = array(
				...
				*/	
				...
				if ($this->user->hasPermission('access', 'extension/module/oct_popup_call_phone')) {
					$sale[] = array(
					'name'	   => 'Заявки на звонок',
					'href'     => $this->url->link('extension/module/oct_popup_call_phone/history_standalone', 'token=' . $this->session->data['token'], true),
					'children' => array()		
					);	
				}
				
				if ($this->user->hasPermission('access', 'extension/module/oct_product_preorder')) {
					$sale[] = array(
					'name'	   => 'Сообщить о наличии',
					'href'     => $this->url->link('extension/module/oct_product_preorder/history_standalone', 'token=' . $this->session->data['token'], true),
					'children' => array()		
					);	
				}
				...
				if ($this->user->hasPermission('access', 'extension/module/giftteaser')) {
					$marketing[] = array(
					'name'	   => 'Подарки к товарам',
					'href'     => $this->url->link('extension/module/giftteaser', 'token=' . $this->session->data['token'], true),
					'children' => array()		
					);	
				}
				...
				if ($this->user->hasPermission('access', 'marketing/coupon')) {	
					$marketing[] = array(
					'name'	   => 'Акционные на главку',
					'href'     => $this->url->link('extension/module/oct_product_tab', 'module_id=35&token=' . $this->session->data['token'], true),
					'children' => array()		
					);	
				}
				...
				//***mf begin
				if ($this->user->hasPermission('access', 'localisation/zone_justin')) {
					$localisation[] = array(
					'name'	   => 'Justin',
					'href'     => $this->url->link('localisation/zone_justin', 'token=' . $this->session->data['token'], true),
					'children' => array()		
					);
				}			
				//***mf end
			...
			if ($this->user->hasPermission('access', 'localisation/city')) {
				...
				if ($this->user->hasPermission('access', 'extension/module/needlessimage')) {
					$tool[] = array(
					'name'	   => 'Удаление картинок',
					'href'     => $this->url->link('extension/module/needlessimage', 'token=' . $this->session->data['token'], true),
					'children' => array()		
					);
				}
				...
				if ($this->user->hasPermission('access', 'report/product_weekviewed')) {
					$report_product[] = array(
					'name'	   => 'Просмотры за неделю',
					'href'     => $this->url->link('report/product_weekviewed', 'token=' . $this->session->data['token'], true),
					'children' => array()	
					);
				}
			    ...
			    $data['epicentr_xml'] = $this->url->link('feed/epicentr_xml', 'token=' . $this->session->data['token'], 'SSL');
