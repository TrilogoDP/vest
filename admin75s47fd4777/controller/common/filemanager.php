...
class ControllerCommonFileManager extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->get['directory'])) {
			$directory = rtrim(DIR_IMAGE . str_replace('*', '', $this->request->get['directory']), '/');//*** . 'data/'
		} else {
		...
		if (substr(str_replace('\\', '/', realpath($directory) . '/' . $filter_name), 0, strlen(DIR_IMAGE)) == DIR_IMAGE) {//*** . 'data'
			// Get directories
        ...
        if (isset($this->request->get['cke'])) {
          $url .= '&cke=' . $this->request->get['cke'];
				...
				$data['images'][] = array(
					...
					'href'  => $this->url->link('common/filemanager', 'token=' . $this->session->data['token'] . '&directory=' . urlencode(utf8_substr($image, utf8_strlen(DIR_IMAGE))) . $url, true)//*** . 'data/'
				);
	...
	public function upload() {
		...
		// Make sure we have the correct directory
		if (isset($this->request->get['directory'])) {
			$directory = rtrim(DIR_IMAGE . $this->request->get['directory'], '/');//***. 'data/'
		} else {
			$directory = DIR_IMAGE . 'data';
		}

		// Check its a directory
		if (!is_dir($directory) || substr(str_replace('\\', '/', realpath($directory)), 0, strlen(DIR_IMAGE)) != DIR_IMAGE) {//***  . 'data'
	...
	public function delete() {
		...
		foreach ($paths as $path) {
			// Check path exsists
			if ($path == DIR_IMAGE || substr(str_replace('\\', '/', realpath(DIR_IMAGE . $path)), 0, strlen(DIR_IMAGE)) != DIR_IMAGE) {//*** . 'data'
