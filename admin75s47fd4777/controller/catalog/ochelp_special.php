	...
	class ControllerCatalogOchelpSpecial extends Controller {
		...
		private function getList() {
			...
			foreach ($results as $result) {
				...
				'active_in_future' => ($result['date_added'] >= date('Y-m-d')),
		...
		private function getForm() {
			...
			if (isset($this->request->post['special_to_category_x'])) {
				$data['special_to_category_x'] = $this->request->post['special_to_category_x'];
				} elseif (isset($special_info)) {
				$data['special_to_category_x'] = $this->model_catalog_ochelp_special->getSpecialCategoriesX($special_info['special_id']);
				} else {
				$data['special_to_category_x'] = array();
			}
			
			if (isset($this->request->post['amount_in_cart'])) {
				$data['amount_in_cart'] = $this->request->post['amount_in_cart'];
				} elseif (isset($special_info)) {
				$data['amount_in_cart'] = $special_info['amount_in_cart'];
				} else {
				$data['amount_in_cart'] = '0';
			}
			
			if (isset($this->request->post['amount_of_present'])) {
				$data['amount_of_present'] = $this->request->post['amount_of_present'];
				} elseif (isset($special_info)) {
				$data['amount_of_present'] = $special_info['amount_of_present'];
				} else {
				$data['amount_of_present'] = '0';
			}
			
			if (isset($this->request->post['discount_percent'])) {
				$data['discount_percent'] = $this->request->post['discount_percent'];
				} elseif (isset($special_info)) {
				$data['discount_percent'] = $special_info['discount_percent'];
				} else {
