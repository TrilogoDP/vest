...
class ControllerCatalogAqeProduct extends Controller {
	...
	public function copy() {
		...
		if (isset($this->request->post['selected']) && $this->validateCopy()) {
			foreach ($this->request->post['selected'] as $product_id) {
				...
				if (!empty($this->request->get['mode']) && $this->request->get['mode'] == 'privacy'){
					$this->load->model('catalog/copy');
					$this->model_catalog_copy->copyPrivacyMatte($product_id);
				} elseif (!empty($this->request->get['mode']) && $this->request->get['mode'] == 'inobi'){
					$this->load->model('catalog/copy2');
					$this->model_catalog_copy2->copyInobiMatte($product_id);				
				} else {
	...
	protected function getList() {
		...
		$data['copyPrivacyMatte'] = $this->url->link('catalog/product/copy', 'mode=privacy&token=' . $this->session->data['token'] . $url, true);
		$data['copyInobiMatte'] = $this->url->link('catalog/product/copy', 'mode=inobi&token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			if (!is_array($columns)) {
				...
				$row['archive'] = ($result['archive'] ? $this->language->get('text_enabled') : $this->language->get('text_disabled'));
				...
				foreach ($columns as $column => $attr) {
					if ($column == 'image') {
						...
						if ((int)$result['archive'] || !$this->config->get('aqe_highlight_status')) {
							$row[$column] = ((int)$result['archive'] ? $this->language->get('text_enabled') : $this->language->get('text_disabled'));
						} else {
							$row[$column] = ((int)$result['archive'] ? ('<span style="color:#FF0000;">' . $this->language->get('text_enabled') . '</span>') :  $this->language->get('text_disabled') );
						}					
			...
			$row['archive_auto'] 	= $result['archive_auto'];
			$row['copied_from'] 	= $result['copied_from'];
			$row['dnup_date'] 		= ($result['dnup_date'] != '0000-00-00') ? $result['dnup_date'] : '';
			$row['price_date'] 		= ($result['price_date'] != '0000-00-00') ? $result['price_date'] : '';
			$row['dnmerchant'] 		= $result['dnmerchant'];			
		...
		$data['archive_select'] = addslashes(json_encode(array(array("id" => "0", "value" => $this->language->get('text_disabled')), array("id" => "1", "value" => $this->language->get('text_enabled')))));
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			if ($results['done']) {
				...
				if (in_array($column, array('model', 'model_marketplace', 'sku', 'upc', 'location', 'attributes', 'discounts', 'images', 'options', 'recurrings', 'related', 'specials', 'descriptions'))) {
					...
					if ((int)$value || !$this->config->get('aqe_highlight_status')) {
						...
						$response['value'] = ((int)$value) ?  '<span style="color:#FF0000;">' . $this->language->get('text_enabled')  . '</span>' : $this->language->get('text_disabled');
					}
					$response['values']['*'] = $response['value'];
