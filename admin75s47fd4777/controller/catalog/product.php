...
//quicksave end
		...
		public function droptoday() {			
			$this->load->model('catalog/product');
			$query = $this->db->query("SELECT product_id FROM oc_product WHERE DATE(date_added) = DATE(NOW()) ORDER BY product_id ASC");
			
			foreach ($query->rows as $row){
				echoLine($row['product_id']);				
...
$this->model_feed_google_merchant_center->saveProduct($this->request->get['product_id'],$this->request->post['google_merchant_product_gender'],$this->request->post['google_merchant_product_age_group'],'');
    ...
    $this->document->addScript('view/javascript/ocfilter/ocfilter.js?v=0.1');
    // OCFilter end
        ...
        $data['entry_option_optsku'] = $this->language->get('entry_option_optsku');
        $data['entry_option_stock'] = $this->language->get('entry_option_stock');
        $data['entry_option_supplier'] = $this->language->get('entry_option_supplier');
        $data['entry_option_model'] = $this->language->get('entry_option_model');
		$data['entry_option_ean'] = $this->language->get('entry_option_ean');
...
if ($this->user->hasPermission('modify', 'module/auto_translator')) {
    $data['at_status'] = $this->config->get('auto_translator_status');
} else { $data['at_status'] = 0; }
$data['at_source'] = $this->config->get('auto_translator_source');
$data['at_api_key'] = $this->config->get('auto_translator_api_key');
$data['at_g_language'] = $this->config->get('auto_translator_g_language');
$data['at_m_language'] = $this->config->get('auto_translator_m_language');
$data['m_token'] = '';
$this->load->model('module/auto_translator');
if ($this->config->get('auto_translator_microsoft_key')) { $token = $this->model_module_auto_translator->getСognitiveMicrosoftToken();
} else { $token = $this->model_module_auto_translator->getMicrosoftToken(); }
if(is_object($token)){ if(isset($token->message)){ $data['microsoft_token_error'] = 'Error: ' . $token->message;
    } else if(isset($token->error->message)){ $data['microsoft_token_error'] = 'Error: ' . $token->error->message;
    } else if(isset($token->error_description)){ $data['microsoft_token_error'] = 'Error: ' . $token->error_description;
    } else { $data['microsoft_token_error'] = 'Error: Failed to generate Microsoft API token!'; }
} else { $data['m_token'] = $token; }
$this->load->language('module/auto_translator');
$data['btn_translate'] = $this->language->get('button_translate');
$data['btn_translate_to'] = $this->language->get('button_translate_to');
$data['btn_translate_all'] = $this->language->get('button_translate_all');
$data['btn_copy'] = $this->language->get('button_copy');
$data['btn_copy_to'] = $this->language->get('button_copy_to');
$data['btn_copy_all'] = $this->language->get('button_copy_all');
$data['btn_copy_name'] = $this->language->get('button_copy_name');
			...
			$data['pricehistory'] = array();
			if (isset($this->request->get['product_id'])){
				$pricehistory = $this->model_catalog_product->getPriceHistory($this->request->get['product_id']);
				
				
				foreach ($pricehistory as $priceline){
					$data['pricehistory'][] = array(
					'suppler_code' => $priceline['suppler_code'],
					'price' 	   => $this->currency->format($priceline['price'], $this->config->get('config_currency')),
					'date'		   => date('d.m.Y', strtotime($priceline['date']))
					
					);				
				}						
			}
			
			$data['salestock_info'] = array(
			'orders' 	=> array(),
			'history' 	=> array(),
			);
			if (isset($this->request->get['product_id'])){
				$orders = $this->model_catalog_product->getOrderByProductId($this->request->get['product_id']);
				
				foreach ($orders as $order){
					$data['salestock_info']['orders'][] = array(
					...
					'date_modified' => date('d.m.Y H:i:s', strtotime($order['date_modified'])),
					);
			...
			$data['entry_custom_yml_mf'] = $this->language->get('entry_custom_yml_mf');
			if (isset($this->request->post['custom_yml_mf'])) {
				$data['custom_yml_mf'] = $this->request->post['custom_yml_mf'];
				} elseif (!empty($product_info)) {
				$data['custom_yml_mf'] = $product_info['custom_yml_mf'];
				} else {
				$data['custom_yml_mf'] = '';
			}
				...
				if (isset($this->request->post['epicentr_name'])) {
					$data['epicentr_name'] = $this->request->post['epicentr_name'];
				} elseif (!empty($product_info)) {
					$data['epicentr_name'] = $product_info['epicentr_name'];
				} else {
					$data['epicentr_name'] = '';
				}


				if (isset($this->request->post['epicentr_name_ua'])) {
					$data['epicentr_name_ua'] = $this->request->post['epicentr_name_ua'];
				} elseif (!empty($product_info)) {
					$data['epicentr_name_ua'] = $product_info['epicentr_name_ua'];
				} else {
					$data['epicentr_name_ua'] = '';
				}


				if (isset($this->request->post['epicentr_quantity'])) {
					$data['epicentr_quantity'] = $this->request->post['epicentr_quantity'];
				} elseif (!empty($product_info)) {
					$data['epicentr_quantity'] = $product_info['epicentr_quantity'];
				} else {
					$data['epicentr_quantity'] = '';
				}
				
				if (isset($this->request->post['epicentr_price'])) {
					$data['epicentr_price'] = $this->request->post['epicentr_price'];
				} elseif (!empty($product_info)) {
					$data['epicentr_price'] = $product_info['epicentr_price'];
				} else {
					$data['epicentr_price'] = '';
				}
						
				if (isset($this->request->post['epicentr_description'])) {
					$data['epicentr_description'] = $this->request->post['epicentr_description'];
				} elseif (!empty($product_info)) {
					$data['epicentr_description'] = $product_info['epicentr_description'];
				} else {
					$data['epicentr_description'] = '';
				}

				if (isset($this->request->post['epicentr_description_ua'])) {
					$data['epicentr_description_ua'] = $this->request->post['epicentr_description_ua'];
				} elseif (!empty($product_info)) {
					$data['epicentr_description_ua'] = $product_info['epicentr_description_ua'];
				} else {
					$data['epicentr_description_ua'] = '';
				}

			
				if (isset($this->request->post['epicentr_status'])) {
					$data['epicentr_status'] = $this->request->post['epicentr_status'];
				} elseif (!empty($product_info)) {
					$data['epicentr_status'] = $product_info['epicentr_status'];
				} else {
					$data['epicentr_status'] = '';
				}
			...
			if (isset($this->request->post['model_marketplace'])) {
				$data['model_marketplace'] = $this->request->post['model_marketplace'];
				} elseif (!empty($product_info)) {
				$data['model_marketplace'] = $product_info['model_marketplace'];
				} else {
				$data['model_marketplace'] = '';	
			}
			...
			if ($product_info){
				$data['productIsAddedFromSupplier'] = $this->model_catalog_product->validateIfProductIsAddedFromSupplier($product_info['product_id']);
			}
			...
			if (isset($this->request->post['orders_90'])) {
				$data['orders_90'] = $this->request->post['orders_90'];
				} elseif (!empty($product_info)) {
				$data['orders_90'] = $product_info['orders_90'];
				} else {
				$data['orders_90'] = false;
			}

			if (isset($this->request->post['orders_180'])) {
				$data['orders_180'] = $this->request->post['orders_180'];
				} elseif (!empty($product_info)) {
				$data['orders_180'] = $product_info['orders_180'];
				} else {
				$data['orders_180'] = false;
			}

			if (!empty($product_info)) {
				$data['dnup_date'] = ($product_info['dnup_date'] != '0000-00-00') ? $product_info['dnup_date'] : '';
				} else {
				$data['dnup_date'] = '0000-00-00';
			}

			if (!empty($product_info)) {
				$data['price_date'] = ($product_info['price_date'] != '0000-00-00') ? $product_info['price_date'] : '';
				} else {
				$data['price_date'] = '0000-00-00';
			}
			
			if (isset($this->request->post['dnmerchant'])) {
				$data['dnmerchant'] = $this->request->post['dnmerchant'];
				} elseif (!empty($product_info)) {
				$data['dnmerchant'] = $product_info['dnmerchant'];
				} else {
				$data['dnmerchant'] = false;
			}
			...
			if (isset($this->request->post['archive_auto'])) {
				$data['archive_auto'] = $this->request->post['archive_auto'];
				} elseif (!empty($product_info)) {
				$data['archive_auto'] = $product_info['archive_auto'];
				} else {
				$data['archive_auto'] = false;
			}
			...
			$data['product_attributes_with_group'] = array();
			$data['product_attributes_no_group'] = array();
			...
			foreach ($product_attributes as $product_attribute) {
				...
				if ($attribute_info) {
					...
					'group_id'						=> $attribute_info['attribute_group_id'],					
					'group'							=> trim($this->model_catalog_attribute_group->getAttributeGroupNameByAttributeID($attribute_info['attribute_group_id'])),
					'group_href'					=> $this->url->link('catalog/attribute_group/edit', 'token=' . $this->session->data['token'] . '&attribute_group_id=' . $attribute_info['attribute_group_id'] , true),
					'product_attribute_description' => $product_attribute['product_attribute_description']
					);

					if ($attribute_array['group']){
						$data['product_attributes_with_group'][] = $attribute_array;
					} else {
						$data['product_attributes_no_group'][] = $attribute_array;
					}
				}
			}

			$data['product_attributes'] = $data['product_attributes_with_group'] + $data['product_attributes_no_group'];
       ...
       || $product_option['type'] == 'image') {
		...
		foreach ($data['product_options'] as $key1 => $options) {
			
			foreach ($options['product_option_value'] as $key2 => $option) {

				$data['product_options'][$key1]['product_option_value'][$key2]['name']= $this->getOptionValueName($option['option_value_id'], $data['option_values'] );
			...
			// SKU_DELETED
			if (isset($this->request->post['product_sku_deleted'])) {
				$data['product_sku_deleted'] = $this->request->post['product_sku_deleted'];
				} elseif (isset($this->request->get['product_id'])) {
				$data['product_sku_deleted'] = $this->model_catalog_product->getProductSKUDeleted($this->request->get['product_id']);
				} else {
				$data['product_sku_deleted'] = array();
			}
			
			if (isset($this->request->post['youtube_single'])) {
				$data['youtube_single'] = $this->request->post['youtube_single'];
				} elseif (!empty($product_info)) {
				$data['youtube_single'] = $product_info['youtube_single'];
				} else {
				$data['youtube_single'] = '';
			}
        ...
        // oct_advanced_options_settings end
			...
			'epicentr_status' => $product_image['epicentr_status'],
			'epicentr_option' => $product_image['epicentr_option'],
        ...
        // oct_advanced_options_settings end
				...
				'video_in_product' => $product_image['video_in_product'],
		...
		protected function validateCopy() {
			...
			///epic xml
				...
				public function getOptionValueName($option_value_id, $option_values){
		...
		//***mf begin
		public function update1c() {
			require(DIR_SYSTEM.'../Mikrof.php');
