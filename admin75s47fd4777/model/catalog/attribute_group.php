...
class ModelCatalogAttributeGroup extends Model {
	...
	public function getAttributeGroupNameByAttributeID($attribute_group_id) {
		$query = $this->db->query("SELECT name FROM " . DB_PREFIX . "attribute_group_description agd WHERE agd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND agd.attribute_group_id = '" . (int)$attribute_group_id . "'");
