...
class ModelCatalogAqeProduct extends Model {
	...
	public function getProducts($data = array()) {
		...
		$anywhere_filters = array(
			...
			'name'      => 'pd.name',
			'model'     => 'p.model',
			'model_marketplace'     => 'p.model_marketplace',
		);
		...
		if (isset($data['filter_special']) && !is_null($data['filter_special'])) {
			$where []= "p.product_id IN (SELECT product_id FROM " . DB_PREFIX . "product_special ps WHERE ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())))";
		}	
	...
	public function quickEditProduct($product_id, $column, $value, $lang_id=null, $data=null) {
		$editable = array('manufacturer', 'image', 'model', 'model_marketplace', 'sku', 'upc', 'ean', 'jan', 'mpn', 'isbn', 'location', 'quantity', 'price', 'weight', 'archive', 'status', 'sort_order', 'tax_class', 'minimum', 'subtract', 'stock_status', 'shipping', 'date_available', 'date_added', 'length', 'width', 'height', 'length_class', 'weight_class', 'points', 'viewed');
		$result = false;
