<?php
	class ModelCatalogOCFilterPage extends Model {
		public function addPage($data) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_page SET keyword = '" . $this->db->escape($data['keyword']) . "', params = '" . $this->db->escape(trim($data['params'], '/')) . "', category_id = '" . (int)$data['category_id'] . "', status = '" . (int)$data['status'] . "', megamenu = '" . (int)$data['megamenu'] . "', sort_order = '" . (int)$data['sort_order'] . "'");
			
			$ocfilter_page_id = $this->db->getLastId();
			
			foreach ($data['page_description'] as $language_id => $value) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_page_description SET ocfilter_page_id = '" . (int)$ocfilter_page_id . "', language_id = '" . (int)$language_id . "', title = '" . $this->db->escape($value['title']) . "', name = '" . $this->db->escape($value['name']) . "', megamenu_name = '" . $this->db->escape($value['megamenu_name']) . "', description = '" . $this->db->escape($value['description']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
			}
		...
		public function editPage($ocfilter_page_id, $data) {
			$this->db->query("UPDATE " . DB_PREFIX . "ocfilter_page SET keyword = '" . $this->db->escape($data['keyword']) . "', params = '" . $this->db->escape(trim($data['params'], '/')) . "', category_id = '" . (int)$data['category_id'] . "', status = '" . (int)$data['status'] . "', megamenu = '" . (int)$data['megamenu'] . "', sort_order = '" . (int)$data['sort_order'] . "' WHERE ocfilter_page_id = '" . (int)$ocfilter_page_id . "'");
		...
		public function getUrlAlias($keyword, $current_page_id = false) {
			...
			if (false /*!$query->num_rows*/) {
				...
				if ($current_page_id) {	
					$query = $this->db->query("SELECT keyword, CONCAT('ocfilter_page_id=', ocfilter_page_id) AS `query` FROM " . DB_PREFIX . "ocfilter_page WHERE LCASE(keyword) = '" . $this->db->escape(utf8_strtolower($keyword)) . "' AND ocfilter_page_id <> '" . (int)$current_page_id . "'");	  
					} else {
					$query = $this->db->query("SELECT keyword, CONCAT('ocfilter_page_id=', ocfilter_page_id) AS `query` FROM " . DB_PREFIX . "ocfilter_page WHERE LCASE(keyword) = '" . $this->db->escape(utf8_strtolower($keyword)) . "' AND ocfilter_page_id <> '" . (int)$current_page_id . "'");
				}
			}
		...
		public function getPageUrlAlias($keyword, $current_page_id = false) {
			$url_alias_info = array();
