<?php
	class ModelCatalogOCFilter extends Model {
		...
		public function addOption($data) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_option SET status = '" . (isset($data['status']) ? (int)$data['status'] : 0) . "', sort_order = '" . (int)$data['sort_order'] . "', type = '" . $this->db->escape($data['type']) . "', selectbox = '" . (isset($data['selectbox']) ? (int)$data['selectbox'] : 0) . "', selection = '" . (int)$data['selection'] . "', highlight = '" . (int)$data['highlight'] . "', faq = '" . (int)$data['faq'] . "', color = '" . (isset($data['color']) ? (int)$data['color'] : 0) . "', image = '" . (isset($data['image']) ? (int)$data['image'] : 0) . "'");
			...
			if (isset($data['ocfilter_option_value']['insert'])) {
				foreach ($data['ocfilter_option_value']['insert'] as $value) {
					...
					$this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_option_value SET option_id = '" . (int)$option_id . "', sort_order = '" . (int)$value['sort_order'] . "', `keyword` = '" . $this->db->escape($this->translit($value['language'][$this->config->get('config_language_id')]['name'])) . "', color = '" . $this->db->escape($value['color']) . "', border = '" . (int)$value['border'] . "', image = '" . $this->db->escape($value['image']) . "'");
			...
			if (isset($data['ocfilter_option_value'])) {
				if (isset($data['ocfilter_option_value']['update'])) {
					foreach ($data['ocfilter_option_value']['update'] as $value_id => $value) {
						...
						$this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_option_value SET option_id = '" . (int)$option_id . "', value_id = '" . (string)$value_id . "', sort_order = '" . (int)$value['sort_order'] . "', `keyword` = '" . $this->db->escape($this->translit($value['language'][$this->config->get('config_language_id')]['name'])) . "', color = '" . $this->db->escape($value['color']) . "', border = '" . (int)$value['border'] . "', image = '" . $this->db->escape($value['image']) . "'");
						
						foreach ($value['language'] as $language_id => $language) {
							$this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_option_value_description SET value_id = '" . (string)$value_id . "', option_id = '" . (int)$option_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($language['name']) . "', alternate_name = '" . $this->db->escape($language['alternate_name']) . "'");
						}
					}
				}
