<?php
	class ModelCatalogOchelpSpecial extends Model {
		...
		private function editProductSpecialData($data){
			...
			if ($product){
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$data['product_id'] . "' AND priority = '" . (int)$this->specialPriority . "'");	
					...
					if ((float)$data['price']){
						...
						priority = '" . (int)$this->specialPriority . "',
						price = '" . (float)$data['price'] . "',
						date_start = '" . $this->db->escape($data['date_added']) . "',
						...
						customer_group_id = '" . $this->config->get('config_customer_group_id') . "',
						priority = '" . (int)$this->specialPriority . "',
						price = '" . (float)($product['price'] - ((float)$product['price']/100*(float)$data['percent'])) . "',
						date_start = '" . $this->db->escape($data['date_added']) . "',
						date_end = '" . $this->db->escape($data['date_end']) . "',
						timer = 0,
						product_special_group_id = 0
		...
		public function getSpecialCategoriesX($special_id){
		
			$special_category_data = array();
			
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "special_to_category_x WHERE special_id = '" . (int)$special_id . "'");
		...
		public function addSpecial($data) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "special SET date_end = '" . $this->db->escape($data['date_end']) . "', sort_order = '" . (int)$data['sort_order'] . "', counter = '" . (int)$data['counter'] . "', total = '" . (int)$data['total'] . "', show_title = '" . (int)$data['show_title'] . "', amount_in_cart = '" . (int)$data['amount_in_cart'] . "', amount_of_present = '" . (int)$data['amount_of_present'] . "', discount_percent = '" . (int)$data['discount_percent'] . "', status = '" . (int)$data['status'] . "', date_added = '" . $this->db->escape($data['date_added']) . "'");
			...
			foreach ($data['special_description'] as $language_id => $value) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "special_description SET special_id = '" . (int)$special_id . "', language_id = '" . (int)$language_id . "', title = '" . $this->db->escape($value['title']) . "', banner = '" . $this->db->escape($value['banner']) . "', label = '" . $this->db->escape($value['label']) . "', label_color = '" . $this->db->escape($value['label_color']) . "', label_bg = '" . $this->db->escape($value['label_bg']) . "', image = '" . $this->db->escape($value['image']) . "', description = '" . $this->db->escape($value['description']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "',  meta_h1 = '" . $this->db->escape($value['meta_h1']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
			}
			...
			$this->db->query("DELETE FROM " . DB_PREFIX . "special_to_category_x WHERE special_id = '" . (int)$special_id . "'");
			
			if (isset($data['special_to_category_x'])) {
				foreach ($data['special_to_category_x'] as $category_id) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "special_to_category_x SET special_id = '" . (int)$special_id . "', category_id = '" . (int)$category_id . "'");
				}
			}
			...
			if (!empty($data['category_id']) || !empty($data['manufacturer_id'])){
				...
				if (!empty($data['manufacturer_id']) && !empty($data['category_id'])){				
					$query = $this->db->query("SELECT DISTINCT product_id FROM " . DB_PREFIX . "product_to_category WHERE category_id = '" . (int)$data['category_id'] . "' AND product_id IN (SELECT DISTINCT product_id FROM "  . DB_PREFIX . "product WHERE manufacturer_id = '" . (int)$data['manufacturer_id'] . "')");
				} elseif (!empty($data['category_id'])) {
					$query = $this->db->query("SELECT DISTINCT product_id FROM " . DB_PREFIX . "product_to_category WHERE category_id = '" . (int)$data['category_id'] . "'");
				} elseif (!empty($data['manufacturer_id'])){
					$query = $this->db->query("SELECT DISTINCT product_id FROM " . DB_PREFIX . "product WHERE manufacturer_id = '" . (int)$data['manufacturer_id'] . "'");
				}
			...
			if (isset($data['special_product'])) {
				foreach ($data['special_product'] as $special_data) {
				
					$special_data['date_end'] = $data['date_end'];
					$special_data['date_added'] = $data['date_added'];
				
					$this->db->query("INSERT INTO " . DB_PREFIX . "special_product SET special_id = '" . (int)$special_id . "', product_id = '" . (int)$special_data['product_id'] . "', price = '" . (float)$special_data['price'] . "',  percent = '" . (float)$data['percent'] . "'");
					if ($status && ((float)$special_data['price'] + (float)$special_data['percent'] > 0)){
						$this->editProductSpecialData($special_data);
					}
				}
			}						
		...
		public function editSpecial($special_id, $data) {
			$this->db->query("UPDATE " . DB_PREFIX . "special SET date_end = '" . $this->db->escape($data['date_end']) . "', date_added = '" . $this->db->escape($data['date_added']) . "', sort_order = '" . (int)$data['sort_order'] . "', counter = '" . (int)$data['counter'] . "', total = '" . (int)$data['total'] . "',  show_title = '" . (int)$data['show_title'] . "', amount_in_cart = '" . (int)$data['amount_in_cart'] . "', amount_of_present = '" . (int)$data['amount_of_present'] . "', discount_percent = '" . (int)$data['discount_percent'] . "', status = '" . (int)$data['status'] . "' WHERE special_id = '" . (int)$special_id . "'");
			...
			foreach ($data['special_description'] as $language_id => $value) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "special_description SET special_id = '" . (int)$special_id . "', language_id = '" . (int)$language_id . "', title = '" . $this->db->escape($value['title']) . "', description = '" . $this->db->escape($value['description']) . "', banner = '" . $this->db->escape($value['banner']) . "', label = '" . $this->db->escape($value['label']) . "', label_color = '" . $this->db->escape($value['label_color']) . "', label_bg = '" . $this->db->escape($value['label_bg']) . "', image = '" . $this->db->escape($value['image']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "',  meta_h1 = '" . $this->db->escape($value['meta_h1']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
			}
			...
			if (isset($data['special_product'])) {
				foreach ($data['special_product'] as $special_data) {
					...
					$this->db->query("INSERT INTO " . DB_PREFIX . "special_product SET special_id = '" . (int)$special_id . "', product_id = '" . (int)$special_data['product_id'] . "', price = '" . (float)$special_data['price'] . "',  percent = '" . (float)$special_data['percent'] . "' ON DUPLICATE KEY UPDATE
					price = '" . (float)$special_data['price'] . "',  percent = '" . (float)$special_data['percent'] . "'
					");
					if ($status && ((float)$special_data['price'] + (float)$special_data['percent'] > 0)){
						$this->editProductSpecialData($special_data);
					}
