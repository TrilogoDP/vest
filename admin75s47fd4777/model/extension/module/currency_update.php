	...
	public function update($force = false) {
		...
		if ($this->config->get('module_currency_update_autoupdate') == 1 || $force) {
			...
			if ($this->config->get('module_currency_update_source') == 'fixer.io') {
				...
				$response = $this->curlRequest('http://data.fixer.io/api/latest?access_key=abfbaa76c3702a5bdd9ba42765858ee3&symbols=UAH,EUR,USD,RUB');								
				...
				if ($response) {
					...
					foreach ($currency_codes as $code) {
						...
						$value = $json->rates->{$base_currency} / $value;
