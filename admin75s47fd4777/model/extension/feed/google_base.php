<?php
	class ModelExtensionFeedGoogleBase extends Model {
		...
		public function import($string) {
			...
			$this->db->query("DELETE FROM " . DB_PREFIX . "feed_manager_taxonomy");
			$lines = explode("\n", $string);
			...
			foreach ($lines as $line) {
				if (substr($line, 0, 1) != '#') {
					...
					if (isset($part[1])) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "feed_manager_taxonomy SET taxonomy_id = '" . (int)$part[0] . "', name = '" . $this->db->escape($part[1]) . "', status = 0");
					}
				}
			}
		}
